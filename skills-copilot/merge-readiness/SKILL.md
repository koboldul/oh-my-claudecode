...
- The runtime (TS hook code) is the backbone: it owns validation, authoritative session state, objective MCQ scoring, and report rendering. The AI owns content generation + MCQ presentation through a direct conversation question.
- Present each MCQ one-per-round through a direct conversation question (deep-interview style). Record each selection via the runtime so it is scored objectively.
...
Present each MCQ one-per-round through a direct conversation question with the option ids/text as choices, then record the human’s selection with the `merge_readiness_record_answer` tool (questionId + optionId). The runtime scores it objectively and either advances to the next question or finalizes the gate (pass / paused / blocked).
...
- [ ] MCQs presented one-per-round through a direct conversation question
- [ ] Each answer correlated from marked direct conversation question output (objective scoring)
