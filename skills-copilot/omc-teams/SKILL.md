...
<!-- Source: skills/omc-teams/SKILL.md; sha256=2798d0ea009f45667f05e815372ef0f987997f4891629fe9a72271af3a17b471 -->
...
- **cursor** CLI: install and authenticate `cursor-agent`; if unavailable, report this setup requirement instead of silently falling back to single-provider execution
...
- Treat Cursor workers as executor-style only. Accept `N:cursor` and `N:cursor:executor`; reject or reframe reviewer, critic, security-reviewer, verdict, or final-approval work onto bundled OMC reviewer Task agents.
...
| `Unsupported agent type`     | Requested agent is not a supported CLI provider | Use `claude`, `codex`, `gemini`, `antigravity`, `grok`, `cursor`, or `copilot`; for bundled OMC Task roles use `/oh-my-claudecode:team` |
