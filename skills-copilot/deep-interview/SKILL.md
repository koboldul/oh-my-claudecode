...
<!-- Source: skills/deep-interview/SKILL.md; sha256=d897923d4c15b4a10ece40e00c51d774c0a31857adb93ab98f7c37e47347f05e -->
...
If any prompt input is too large, summarize it first and then continue from the summary. Do not ask the next a direct conversation question, score ambiguity, or hand off to execution from an over-budget raw transcript.
...
Ask the user directly in the conversation with the generated question. Present it clearly with the current ambiguity context:
...
**Scoring prompt** (use deep model, temperature 0.1 for consistency):
...
1. **Generate the specification** using deep model with the prompt-safe transcript. If the full interview transcript or initial context is too large, include the summary plus all concrete decisions, acceptance criteria, unresolved gaps, and ontology snapshots; never overflow the prompt with raw oversized context.
...
**Autoresearch override:** if `--autoresearch` is active, skip the standard execution options below. The only valid bridge is the `skill("autoresearch")` handoff described above. The `omc autoresearch` CLI is a hard-deprecated shim and must not be used for execution.

After the spec is written, mark it `pending approval` and present execution options through a direct conversation question. Until the user selects an execution option, the deep-interview module MUST NOT run mutation-oriented shell commands, edit source files, commit, push, open PRs, invoke execution skills, or delegate implementation tasks:
...
**IMPORTANT:** On explicit execution selection, **MUST** invoke the selected skill by its explicit name. Do NOT implement directly. The deep-interview agent is a requirements agent, not an execution agent. If oversized initial context was summarized, pass the spec and prompt-safe summary forward, not the raw oversized source material. Without explicit execution selection, stop with the spec marked `pending approval`.
...
<Tool_Usage>
- Ask the user directly in the conversation for each interview question — provides clickable UI with contextual options
- Preserve the direct conversation question path for OMC-native interaction; do not introduce OMX-only structured-question transport into this skill
- Use `Task(agent_type="oh-my-claudecode:explore")` for brownfield codebase exploration (run BEFORE asking user about codebase)
- Use deep model (temperature 0.1) for ambiguity scoring — consistency is critical
...
- Invoke the selected skill by its explicit name to bridge to execution modes only after explicit execution approval — never implement directly
...
- [ ] Execution bridge presented through a direct conversation question
- [ ] Selected execution mode invoked by its explicit skill name only after explicit execution approval (never direct implementation)
