...
<!-- Source: skills/ultrawork/SKILL.md; sha256=79d3661d0ef21a2bcfbb3cd642bee578268649577532364707c5dc4a397484e7 -->
...
6. **Route to correct tiers**:
   - Simple lookups/definitions: LOW tier (Quick)
   - Standard implementation: MEDIUM tier (Standard)
   - Complex analysis/refactoring: HIGH tier (Deep)
...
<Tool_Usage>
- Use `Task(agent_type="oh-my-claudecode:executor", ...)` for simple changes
- Use `Task(agent_type="oh-my-claudecode:executor", ...)` for standard work
- Use `Task(agent_type="oh-my-claudecode:executor", ...)` for complex work
...
```
Task(agent_type="oh-my-claudecode:executor", prompt="Add missing type export for Config interface")
Task(agent_type="oh-my-claudecode:executor", prompt="Implement the /api/users endpoint with validation")
Task(agent_type="oh-my-claudecode:executor", prompt="Add integration tests for the auth middleware")
```
...
```
Task(agent_type="oh-my-claudecode:executor", prompt="npm install && npm run build", run_in_background=true)
Task(agent_type="oh-my-claudecode:executor", prompt="Update the README with new API endpoints")
```
...
```
result1 = Task(agent_type="oh-my-claudecode:executor", prompt="Add type export")  # wait...
result2 = Task(agent_type="oh-my-claudecode:executor", prompt="Implement endpoint")     # wait...
result3 = Task(agent_type="oh-my-claudecode:executor", prompt="Add tests")              # wait...
```
...
```
Task(agent_type="oh-my-claudecode:executor", prompt="Add a missing semicolon")
```
Why bad: Deep is expensive overkill for a trivial fix. Use executor with Quick instead.
</Bad>
</Examples>
