...
<!-- Source: skills/deep-dive/SKILL.md; sha256=c08ce5be26385d34fbc79aae2a28d88e3641fda3c370033abbb420d71ab5d0fc -->
...
3. **Detect brownfield vs greenfield**:
   - Run `explore` agent (quick): check if cwd has existing source code, package files, or git history
...
Present the 3 hypotheses to the user through a direct conversation question for confirmation (1 round only):
...
If the guidance gate does not apply, or the pre-flight passes, present execution options through a direct conversation question:
...
**IMPORTANT:** On execution selection, **MUST** invoke the selected skill by its explicit name with `spec_path`. Do NOT implement directly. The deep-dive skill is a requirements pipeline, not an execution agent.
...
<Tool_Usage>
- Ask the user directly in the conversation for lane confirmation (Phase 2) and each interview question (Phase 4)
- Use `Task(agent_type="oh-my-claudecode:explore")` for brownfield codebase exploration (Phase 1)
...
- Invoke the selected skill by its explicit name to bridge to execution modes (Phase 5) — never implement directly
...
- [ ] Phase 2 confirms hypotheses through a direct conversation question (1 round)
...
The execution bridge passes `spec_path` explicitly to downstream skills. autopilot/ralph/team receive the path as context in the selected skill invocation, so filename-pattern matching is not required.
