...
<!-- Source: skills/local-build-reminder/SKILL.md; sha256=99e99de22da7907a1244fd1842c5d2907982638381d9ff49164e670ad36c68c1 -->

# Local Build Reminder

**Always-on reminder for OMC fork development.** When OMC is running in local
mode (HUD shows `[OMC#X.Y.ZL]` with an `L` suffix), GitHub Copilot CLI loads compiled
...
3. The user is about to restart GitHub Copilot CLI and the working tree has TS edits with no rebuild.
...
> GitHub Copilot CLI — `dist/` won't reflect the change otherwise.
...
- The user is in a GitHub Copilot CLI session that isn't running OMC locally
  (no `L` in the HUD).
...
triggers a rebuild within a second, so `restart GitHub Copilot CLI` is all that's
needed afterwards.
