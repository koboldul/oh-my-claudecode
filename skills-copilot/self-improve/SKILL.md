...
<!-- Source: skills/self-improve/SKILL.md; sha256=faba8ae209767582d1133741cc99ad7e6781c82d57b17bfe29213413f7935fc4 -->
<!-- Support channel: scripts/validate.sh; git-mode=100755; npm-tar-mode=0644-or-0755; invocation=bash "$VALIDATE_SH"; windows=Bash via Git Bash, WSL, or MSYS2 -->
...
|------|------|-----------|-------|
| Research | Codebase analysis + hypothesis generation | general-purpose Agent | deep |
| Planning | Hypothesis → structured plan | oh-my-claudecode:planner | deep |
| Architecture Review | 6-point plan review | oh-my-claudecode:architect | deep |
| Critic Review | Harness rule enforcement | oh-my-claudecode:critic | deep |
| Execution | Implement plan + run benchmark | oh-my-claudecode:executor | deep |
...
| Benchmark Setup | Create + validate benchmark | custom agent | deep |
...
6. If benchmark not set → read `si-benchmark-builder.md` from this skill directory, launch Task(agent_type="general-purpose", prompt="<contents of the referenced file>"). The agent surveys the repo, creates or wraps a benchmark, validates 3x, and records baseline.
...
Launch Task(agent_type="general-purpose", prompt="<contents of `si-researcher.md`>").
...
Spawn N `oh-my-claudecode:planner`(model=deep) agents in parallel (N = `number_of_agents` from settings).
...
For each approved plan, spawn `oh-my-claudecode:executor`(model=deep) in parallel.
...
- **Override instructions**: Implement the plan faithfully, run `bash "$VALIDATE_SH"` before benchmarking, run the benchmark command, produce Benchmark Result JSON as output.
