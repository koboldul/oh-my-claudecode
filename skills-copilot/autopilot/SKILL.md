...
<!-- Source: skills/autopilot/SKILL.md; sha256=89ab935a4b7d45dc5a382e3ba837b595ee1c25d8044f54177941401fe14a431f -->
...
1. **Phase 0 - Expansion**: Turn the user's idea into a detailed spec
   ...
   - **Otherwise**: Analyst (Deep) extracts requirements, Architect (Deep) creates technical specification
...
2. **Phase 1 - Planning**: Create an implementation plan from the spec
   ...
   - Architect (Deep): Create plan (direct mode, no interview)
   - Critic (Deep): Validate plan
   - Output: `.omc/plans/autopilot-impl.md`

3. **Phase 2 - Execution**: Implement the plan using Ralph + Ultrawork
   - Executor (Quick): Simple tasks
   - Executor (Standard): Standard tasks
...
<Tool_Usage>
- Use `Task(agent_type="oh-my-claudecode:architect", ...)` for Phase 4 architecture validation
- Use `Task(agent_type="oh-my-claudecode:security-reviewer", ...)` for Phase 4 security review
- Use `Task(agent_type="oh-my-claudecode:code-reviewer", ...)` for Phase 4 quality review
...
or the plugin-scoped slash compatibility surface:
...
- Keep reviewer, critic, security-review, validation verdict, and final approval roles on bundled OMC reviewer Task agents unless explicit safe support is added later.
- Cursor requires the `cursor-agent` CLI to be installed and authenticated. If `cursor-agent` is unavailable, report that setup requirement instead of silently falling back to single-provider execution.
