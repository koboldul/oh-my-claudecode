...
<!-- Source: skills/ralph/SKILL.md; sha256=7de3f960f37d7ff886597b79d7865330e64c6aeb802d18546373c6e0bed393f3 -->
...
3. **Implement the current story**:
   - Delegate to specialist agents at appropriate tiers:
     - Simple lookups: LOW tier (Quick) -- "What does this function return?"
     - Standard work: MEDIUM tier (Standard) -- "Add error handling to this module"
     - Complex analysis: HIGH tier (Deep) -- "Debug this race condition"
...
7. **Reviewer verification** (tiered, against acceptance criteria):
   - <5 files, <100 lines with full tests: STANDARD tier minimum (architect-medium / Standard)
   - Standard changes: STANDARD tier (architect-medium / Standard)
   - > 20 files or security/architectural changes: THOROUGH tier (architect / Deep)
...
- **ai-slop-cleaner is a SKILL, not an agent.** Do NOT call it via `Task(agent_type="oh-my-claudecode:ai-slop-cleaner")` — that subagent type does not exist and the call will fail with "Agent type not found". If you see that error, retry with the Skill tool — do NOT substitute a similarly-named agent like `code-simplifier` as a "closest match".
...
- Use `Task(agent_type="oh-my-claudecode:architect", ...)` for architect verification cross-checks when changes are security-sensitive, architectural, or involve complex multi-system integration
- Use `Task(agent_type="oh-my-claudecode:critic", ...)` when `--critic=critic`
...
- **Skill vs agent invocation**: `ai-slop-cleaner` is a skill, invoke via `skill("ai-slop-cleaner")`. `architect`, `critic`, `executor` etc. are agents, invoke via `Task(agent_type="oh-my-claudecode:<name>")`. If you ever get "Agent type ... not found" for an `oh-my-claudecode:<name>` identifier, the item is a skill — retry with the Skill tool. Do NOT substitute a similarly-named agent as a "closest match".
  </Tool_Usage>
...
Task(agent_type="oh-my-claudecode:executor", prompt="Add type export for UserConfig")
Task(agent_type="oh-my-claudecode:executor", prompt="Implement the caching layer for API responses")
Task(agent_type="oh-my-claudecode:executor", prompt="Refactor auth module to support OAuth2 flow")
...
Task(agent_type="oh-my-claudecode:executor", prompt="Add type export") → wait →
Task(agent_type="oh-my-claudecode:executor", prompt="Implement caching") → wait →
Task(agent_type="oh-my-claudecode:executor", prompt="Refactor auth")
