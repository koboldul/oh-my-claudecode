...
let stagedCleanupRoot;
let stagedPluginRoot;
let stagedSessionEnd;
let timingPluginRoot;
let timingSessionEnd;
let timingWikiSessionEnd;
let residentRuntimeRoot;
const workerPidFiles = new Set();
function writeModule(path, source) {
    mkdirSync(dirname(path), { recursive: true });
    writeFileSync(path, source, 'utf8');
}
function emitSessionEndClosure(outDir) {
    ...
    diagnostics.push(...emit.diagnostics.filter(diagnostic => diagnostic.category === ts.DiagnosticCategory.Error));
    ...
    const patched = processUtils.replace(/export function getProcessStartIdentitySync\(pid\) \{\r?\n/, "export function getProcessStartIdentitySync(pid) {\n    return `session-end-test:${pid}`;\n");
...
beforeAll(() => {
    ...
    for (const filename of [
        ...
        copyFileSync(join(REPO_ROOT, 'scripts', filename), join(scriptsDir, filename));
    }
    mkdirSync(join(stagedPluginRoot, 'hooks'), { recursive: true });
    copyFileSync(join(REPO_ROOT, 'hooks', 'hooks.json'), join(stagedPluginRoot, 'hooks', 'hooks.json'));
    mkdirSync(join(stagedPluginRoot, '.claude-plugin'), { recursive: true });
    writeModule(join(stagedPluginRoot, 'package.json'), JSON.stringify({ type: 'module' }));
    writeModule(join(stagedPluginRoot, '.claude-plugin', 'plugin.json'), '{}');
    symlinkSync(join(REPO_ROOT, 'node_modules'), join(stagedPluginRoot, 'node_modules'), process.platform === 'win32' ? 'junction' : 'dir');
...
function runUntilClose(script, cwd, input, ceilingMs = HOST_PROCESS_WATCHDOG_MS, extraEnv = {}, pluginRoot = stagedPluginRoot) {
    return new Promise((resolve) => {
        ...
        const child = spawn(process.execPath, [join(pluginRoot, 'scripts', 'run.cjs'), script], {
            cwd,
            env: {
                ...process.env,
                HOME: home,
...
function expectPromptExit(result, output = '{"continue":true}\n', diagnostic) {
...
function readSessionEndTiming(path) {
    return JSON.parse(readFileSync(path, 'utf8'));
}
function expectRunnerAdmissionDuration(timing, diagnostic) {
...
function reportTimingSummary(label, samples) {
    if (process.env.OMC_SESSION_END_TEST_REPORT_TIMINGS !== '1'
        || samples.length === 0) {
        return;
    }
    ...
    process.stdout.write(`[session-end-timing] ${label} calls=${samples.length} `
...
function configureDeferredAdapters(cwd, callbackPath = join(cwd, 'callback-{session_id}.md')) {
    const configDir = join(cwd, '.claude');
    mkdirSync(configDir, { recursive: true });
...
async function readJsonEventually(path, ceilingMs = DETACHED_WORKER_CEILING_MS) {
...
async function readTextEventually(path, ceilingMs = DETACHED_WORKER_CEILING_MS) {
    const deadline = Date.now() + ceilingMs;
    let lastError;
    while (Date.now() < deadline) {
        try {
...
async function readBufferEventually(path, ceilingMs = DETACHED_WORKER_CEILING_MS) {
    const deadline = Date.now() + ceilingMs;
    let lastError;
    while (Date.now() < deadline) {
        try {
...
async function waitForTerminalManifest(cwd, sessionId) {
...
async function waitForTerminalCallback(cwd, sessionId, callbackPath) {
    const manifestPath = sessionEndJobPath(cwd, sessionId);
    const deadline = Date.now() + DETACHED_WORKER_CEILING_MS;
    ...
    throw new Error(`detached SessionEnd worker did not complete its callback; last manifest: ${JSON.stringify(lastManifest)}`);
}
...
function workerTracking(cwd, label) {
...
describe('SessionEnd run.cjs runner/admission latency and detached completion', () => {
    ...
    function createProject(label = 'project') {
    ...
    function residentEnvironment(extra = {}) {
        return {
            NODE_ENV: 'test',
    ...
    async function ensureResident(cwd, sessionId, extra = {}, pluginRoot = stagedPluginRoot) {
        const { ensureSessionEndResident } = await import(pathToFileURL(join(pluginRoot, 'scripts', 'lib', 'session-end-resident-control.mjs')).href);
    ...
    it.each([
    ...
    ])('%s reports <=500ms runner duration on every cold empty open-stdin run', async (name, script) => {
        const samples = [];
        for (let index = 0; index < TIMING_RUNS; index += 1) {
            ...
            const result = await runUntilClose(script(), cwd, undefined, HOST_PROCESS_WATCHDOG_MS, {
                NODE_ENV: 'test',
                OMC_SESSION_END_TEST_IPC_RECEIPT: receipt,
            }, timingPluginRoot);
            expectPromptExit(result, '{"continue":true,"suppressOutput":true}\n');
    ...
    it.each([
        ['session-end', () => timingSessionEnd],
        ['wiki-session-end', () => timingWikiSessionEnd],
    ])('%s reports <=500ms runner/admission duration on every repeated configured run', async (name, script) => {
        const cwd = createProject(`configured-${name}`);
        configureDeferredAdapters(cwd);
        const sessionId = `configured-${name}`;
        const residentEnv = await ensureResident(cwd, sessionId, {}, timingPluginRoot);
        ...
        for (let index = 0; index < TIMING_RUNS; index += 1) {
            ...
            const result = await runUntilClose(script(), cwd, raw, HOST_PROCESS_WATCHDOG_MS, {
                ...residentEnv,
                ...tracking.env,
                OMC_SESSION_END_TEST_IPC_RECEIPT: receipt,
            }, timingPluginRoot);
        ...
        expect(await readBufferEventually(tracking.rawReceipt)).toEqual(Buffer.from(validSessionEndInput(cwd, sessionId)));
    ...
    it('completes callbacks through a USERNA~1 path and consumes the resident spool', async () => {
        ...
        expectPromptExit(await runUntilClose(stagedSessionEnd, cwd, raw, HOST_PROCESS_WATCHDOG_MS, { ...residentEnv, ...core.env }));
        ...
        workerPids.add(pid);
        const completion = await readJsonEventually(core.completionFile);
    ...
    it('keeps the resident alive through producer grace and reaches terminal completion', async () => {
        ...
        configureDeferredAdapters(cwd, callbackPath);
        const residentEnv = await ensureResident(cwd, sessionId);
        expectPromptExit(await runUntilClose(stagedSessionEnd, cwd, validSessionEndInput(cwd, sessionId), HOST_PROCESS_WATCHDOG_MS, {
            ...residentEnv,
        }));
        await waitForTerminalCallback(cwd, sessionId, callbackPath);
    }, 75_000);
    it('keeps configured callbacks, proxies, and custom CA out of the foreground process', async () => {
        ...
        const residentEnv = await ensureResident(cwd, sessionId);
        const result = await runUntilClose(timingSessionEnd, cwd, validSessionEndInput(cwd, sessionId), HOST_PROCESS_WATCHDOG_MS, {
            ...residentEnv,
            ...tracking.env,
    ...
    it('restores a resident environment value before processing the next admission', async () => {
        ...
        try {
            ...
            expectPromptExit(await runUntilClose(timingSessionEnd, cwd, validSessionEndInput(cwd, sessionId), HOST_PROCESS_WATCHDOG_MS, {
                ...residentEnv,
            ...
            expectPromptExit(await runUntilClose(timingSessionEnd, cwd, nextRaw, HOST_PROCESS_WATCHDOG_MS, nextEnv, timingPluginRoot));
    ...
    it('lets wiki-session-end return without waiting for a live wiki lock', async () => {
        ...
        const residentEnv = await ensureResident(cwd, sessionId);
        expectPromptExit(await runUntilClose(timingWikiSessionEnd, cwd, validSessionEndInput(cwd, sessionId), HOST_PROCESS_WATCHDOG_MS, { ...residentEnv, ...tracking.env }, timingPluginRoot));
        const pid = Number(await readTextEventually(tracking.pidFile));
        workerPids.add(pid);
        expect(await readJsonEventually(tracking.completionFile)).toMatchObject({
            pid,
    ...
    it('recovers a durable no-resident spool on the next resident start', async () => {
        ...
        expectPromptExit(await runUntilClose(stagedSessionEnd, cwd, validSessionEndInput(cwd, sessionId), HOST_PROCESS_WATCHDOG_MS, env));
    ...
    it('serializes concurrent core/wiki admission in one resident', async () => {
        ...
        const [core, wiki] = await Promise.all([
            runUntilClose(stagedSessionEnd, cwd, raw, HOST_PROCESS_WATCHDOG_MS, { ...env, OMC_SESSION_END_TEST_IPC_RECEIPT: coreReceipt }),
            runUntilClose(timingWikiSessionEnd, cwd, raw, HOST_PROCESS_WATCHDOG_MS, { ...env, OMC_SESSION_END_TEST_IPC_RECEIPT: wikiReceipt }),
        ]);
        ...
        expectRunnerAdmissionDuration(coreTiming, JSON.stringify({ hostElapsedMs: core.elapsedMs, timing: coreTiming }));
        expectRunnerAdmissionDuration(wikiTiming, JSON.stringify({ hostElapsedMs: wiki.elapsedMs, timing: wikiTiming }));
    ...
    it('authenticates both directions and rejects replay, bad MAC, stale gates, and traversal', async () => {
        ...
        const env = await ensureResident(cwd, sessionId);
        const ipc = requireFromTest(join(stagedPluginRoot, 'scripts', 'lib', 'session-end-ipc.cjs'));
        ...
        const publish = (suffix) => ipc.publishSessionEndFrame(context, {
            producer: 'core',
        ...
        const firstRequest = ipc.buildSignedRequest(control, context, firstPublished, fullEnv);
        ...
        const badMacRequest = ipc.buildSignedRequest(control, context, publish('-bad-mac'), fullEnv);
        badMacRequest.mac = '00'.repeat(32);
        expect((await ipc.exchange(control, badMacRequest)).acknowledged).toBe(false);
        const staleRequest = ipc.buildSignedRequest(control, context, publish('-stale'), fullEnv);
        staleRequest.timestamp = Date.now() - 60_000;
        staleRequest.mac = ipc.signObject(control.token, staleRequest);
        expect((await ipc.exchange(control, staleRequest)).acknowledged).toBe(false);
        const traversalRequest = ipc.buildSignedRequest(control, context, publish('-traversal'), fullEnv);
        traversalRequest.spool = '../foreign.frame';
        traversalRequest.mac = ipc.signObject(control.token, traversalRequest);
    ...
    it('recovers a crash after durable claim even when the ACK is lost', async () => {
        ...
        const ipcResult = JSON.parse(readFileSync(ipcReceipt, 'utf8'));
    ...
    it('leaves the durable processing claim quarantined after retry exhaustion', async () => {
        ...
        const paths = ipc.spoolPaths(context, spoolPath.split(/[\\/]/).at(-1), ipcResult.eventId);
        const deadline = Date.now() + 45_000;
        let claim = null;
        while (Date.now() < deadline) {
            try {
                claim = JSON.parse(readFileSync(paths.claimPath, 'utf8'));
                if (claim.attempts === 3 && claim.lastError)
                    break;
            }
            catch {
    ...
    it('keeps exactly 50 resident calls at <=500ms runner/admission duration with bounded authenticated ACKs and zero child creation', async () => {
        ...
        for (let index = 0; index < SEQUENTIAL_GATE_CALLS; index += 1) {
            ...
            const result = await runUntilClose(timingSessionEnd, cwd, raw, HOST_PROCESS_WATCHDOG_MS, {
                ...env,
                OMC_SESSION_END_TEST_IPC_RECEIPT: receipt,
            }, timingPluginRoot);
            const timing = readSessionEndTiming(receipt);
            ...
            expectPromptExit(result, '{"continue":true}\n', diagnostic);
            expect(timing.acknowledged, diagnostic).toBe(true);
