...
function runMissionProcess(runnerPath, method, cwd, sessionId, payload = {}, env = {}) {
...
function buildMissionRunner(cwd) {
...
describe('mission board state tracking', () => {
    ...
    it('preserves concurrent session-agent read-modify-write transactions', async () => {
        ...
        await Promise.all(Array.from({ length: 19 }, (_, index) => runMissionProcess(runnerPath, 'recordMissionAgentStart', cwd, sessionId, {
            sessionId,
            agentId: `agent-${index}`,
            agentType: 'oh-my-claudecode:executor',
            parentMode: 'ultrawork',
        ...
        await Promise.all(Array.from({ length: 19 }, (_, index) => runMissionProcess(runnerPath, 'recordMissionAgentStop', cwd, sessionId, {
            sessionId,
            agentId: `agent-${index}`,
            success: true,
        ...
        expect(completed?.missions[0]?.agents.every((agent) => agent.status === 'done')).toBe(true);
    ...
    it('serializes concurrent reader/writer legacy migration without clobbering', async () => {
        ...
        const [readerOutput] = await Promise.all([
            runMissionProcess(runnerPath, 'readMissionBoardState', cwd, sessionId, {}, migrationEnv),
            runMissionProcess(runnerPath, 'recordMissionAgentStart', cwd, sessionId, {
                sessionId,
        ...
        const readerState = JSON.parse(readerOutput);
        expect(readerState.missions.some((mission) => mission.id === 'legacy-mission')).toBe(true);
        const paths = resolveSessionStatePaths('mission-state', sessionId, cwd);
        const persisted = JSON.parse(readFileSync(paths.sessionScoped, 'utf8'));
        expect(persisted.missions.some((mission) => mission.id === 'legacy-mission')).toBe(true);
        expect(persisted.missions.some((mission) => mission.agents.some((agent) => agent.ownership === 'writer-agent'))).toBe(true);
        expect(readdirSync(join(paths.sessionScoped, '..')).some((name) => name.includes('.migrating.'))).toBe(false);
    });
