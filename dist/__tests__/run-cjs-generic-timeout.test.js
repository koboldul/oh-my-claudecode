...
describe('run.cjs generic hook timeout supervisor', () => {
    ...
    it.each(['PermissionRequest', 'PreToolUse'])('fails closed when a manifest-resolved %s hook times out', async (event) => {
        await expect(withWatchdog(runCjs.runGenericChild(SLOW_EXIT, [], 25, { event, timeoutMs: 525 }))).resolves.toBe(2);
    });
    it('keeps optional manifest-resolved timeout and spawn failures fail-open', async () => {
        ...
        try {
            await expect(withWatchdog(runCjs.runGenericChild(SLOW_EXIT, [], 25, { event: 'PostToolUse', timeoutMs: 525 }))).resolves.toBe(0);
            const originalExecPath = process.execPath;
            ...
            try {
                await expect(withWatchdog(runCjs.runGenericChild(SLOW_EXIT, [], 2000, { event: 'PostToolUse', timeoutMs: 2500 }))).resolves.toBe(0);
            }
            finally {
                Object.defineProperty(process, 'execPath', {
                    configurable: true,
    ...
    it.each(['PermissionRequest', 'PreToolUse'])('fails closed when a manifest-resolved %s hook cannot spawn', async (event) => {
        ...
        try {
            await expect(withWatchdog(runCjs.runGenericChild(join(directory, 'missing.cjs'), [], 2000, { event, timeoutMs: 2500 }))).resolves.toBe(2);
        }
        finally {
            Object.defineProperty(process, 'execPath', {
                configurable: true,
    ...
    it.each(['PermissionRequest', 'PreToolUse'])('accepts only a successful manifest-resolved %s child exit', async (event) => {
        ...
        try {
            await expect(withWatchdog(runCjs.runGenericChild(success, [], 2000, { event, timeoutMs: 2500 }))).resolves.toBe(0);
        }
        finally {
            rmSync(directory, { recursive: true, force: true });
        }
    });
    it.each(['PermissionRequest', 'PreToolUse'])('maps every abnormal manifest-resolved %s child termination to exit 2', async (event) => {
        ...
        try {
            for (const testCase of cases) {
                ...
                const status = await withWatchdog(runCjs.runGenericChild(fixture, [], 2000, { event, timeoutMs: 2500 }));
    ...
    it.each([
        ...
        ['PreToolUse', 2],
        ['PostToolUse', 0],
    ])('uses manifest policy for %s runner signals (exit %i, POSIX)', async (event, expectedStatus) => {
        ...
        writeFileSync(target, `require('node:fs').writeFileSync(${JSON.stringify(pidfile)}, String(process.pid)); setInterval(() => {}, 1e9);`);
        ...
        try {
            ...
            const runnerExit = new Promise(resolve => {
