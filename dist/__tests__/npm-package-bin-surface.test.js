...
import { COPILOT_NATIVE_HOOK_EVENTS, PLUGIN_JSON_PATH, listSourceControlledPackageFiles, readMcpServersFromPath, readPluginMcpServers, referencesCopilotHooksManifest, referencesRootMcpConfig, referencesStandardHooksManifest, } from "./npm-package-surface-helpers.js";
...
function tarPermissions(archivePath, entryPath, compressed) {
    const output = execFileSync("tar", [compressed ? "-tvzf" : "-tvf", archivePath, entryPath], { encoding: "utf-8" }).trim();
    return output.split(/\s+/)[0] ?? "";
}
function tarFileMode(permissions) {
...
function validateWithExplicitBash(scriptPath) {
    ...
    if (process.platform === "win32") {
        ...
        const syntax = spawnSync("wsl.exe", ["bash", "-n", translatedPath], {
            encoding: "utf-8",
            windowsHide: true,
        });
...
function createIsolatedPackWorkspace(workspacePath, snapshotPath) {
    ...
    const realIndexPath = resolve(PACKAGE_ROOT, execFileSync("git", ["rev-parse", "--git-path", "index"], {
        cwd: PACKAGE_ROOT,
        encoding: "utf-8",
        stdio: ["ignore", "pipe", "ignore"],
    }).trim());
...
function getPackedPackage() {
    ...
    try {
        ...
        const stdout = execFileSync(npmCommand, npmCommandArgs, {
        ...
        packedPackageCache = {
            ...
            copilotPluginJson: JSON.parse(readFileSync(join(extractedPackageRoot, "plugin.json"), "utf-8")),
            copilotCapabilityMatrix: JSON.parse(readFileSync(join(extractedPackageRoot, "prompt-assets", "copilot-capability-matrix.json"), "utf-8")),
...
describe("npm package bin surface regression", () => {
    ...
    it("packs the built runtime closure and preserves source payload bytes", () => {
        const extractedPackageRoot = join(packDirCache, "package");
        const surface = collectPluginRuntimeClosure(extractedPackageRoot);
        for (const relativePath of surface.requiredPaths) {
            expect(packedPackageFixture.files.has(relativePath), relativePath).toBe(true);
            if (surface.generatedRoots.some((root) => relativePath === root || relativePath.startsWith(`${root}/`)) ||
    ...
    it("typechecks the packed team declaration closure with skipLibCheck disabled", () => {
        ...
        const packedNodeModules = join(packedPackageFixture.extractedPackageRoot, "node_modules");
        try {
    ...
    it("typechecks the supported team export from a clean tarball install", () => {
        ...
        execFileSync(npmCommand, npmArgs, { cwd: consumerRoot, stdio: "pipe" });
    ...
    it("packs dependency-closed Copilot HUD setup and runtime entrypoints", () => {
        ...
        for (const relativePath of [
            ...
            expect(readFileSync(join(packedRoot, relativePath), "utf8")).not.toMatch(/(?:from\s+["']jsonc-parser["']|require\(["']jsonc-parser["']\))/);
        }
        ...
        try {
            ...
            const setupPath = join(packedRoot, "bridge", "copilot-hud-setup.mjs");
            const setup = spawnSync(process.execPath, [setupPath, "setup", "--json"], { cwd: workspace, encoding: "utf8", env });
            ...
            const payload = JSON.parse(readFileSync(join(PACKAGE_ROOT, "src", "__tests__", "fixtures", "hooks", "copilot-1.0.72-1", "statusLine.json"), "utf8"));
            payload.workspace.current_dir = workspace;
            payload.transcript_path = join(workspace, "missing.jsonl");
            const render = spawnSync(process.execPath, [result.wrapperPath], {
                cwd: workspace,
                encoding: "utf8",
                env,
    ...
    it("keeps packed plugin and MCP manifests wired to exact standard entrypoints", () => {
        ...
        expect(packedPackageFixture.copilotPluginJson).toEqual(JSON.parse(readFileSync(join(process.cwd(), "plugin.json"), "utf-8")));
        expect(packedPackageFixture.copilotPluginJson.agents).toBe("./agents-copilot/");
        expect(packedPackageFixture.copilotPluginJson.commands).toBe("./commands-copilot/");
        expect([...(packedPackageFixture.copilotPluginJson.skills ?? [])].sort()).toEqual(packedPackageFixture.copilotCapabilityMatrix.skills.map(({ id }) => `./skills-copilot/${id}/`).sort());
        expect(referencesStandardHooksManifest(packedPackageFixture.pluginJson.hooks)).toBe(true);
        expect(referencesCopilotHooksManifest(packedPackageFixture.copilotPluginJson.hooks)).toBe(true);
        expect(referencesRootMcpConfig(packedPackageFixture.pluginJson.mcpServers)).toBe(true);
        const packedCopilotHooks = JSON.parse(readFileSync(join(packedPackageFixture.extractedPackageRoot, "hooks", "copilot-hooks.json"), "utf-8"));
        const sourceCopilotHooks = JSON.parse(readFileSync(join(process.cwd(), "hooks", "copilot-hooks.json"), "utf-8"));
        expect(packedCopilotHooks).toEqual(sourceCopilotHooks);
        expect(packedCopilotHooks.version).toBe(1);
        expect(Object.keys(packedCopilotHooks.hooks ?? {})).toEqual(COPILOT_NATIVE_HOOK_EVENTS);
    ...
    it("packs every generated Copilot prompt and its capability inventory", () => {
        ...
        const sourceCapabilityMatrix = JSON.parse(readFileSync(join(PACKAGE_ROOT, "prompt-assets", "copilot-capability-matrix.json"), "utf-8"));
        const executableSupportPath = "skills-copilot/self-improve/scripts/validate.sh";
        const selfImprove = copilotCapabilityMatrix.skills.find(({ id }) => id === "self-improve");
        expect(copilotCapabilityMatrix).toEqual(sourceCapabilityMatrix);
        expect(files.has("prompt-assets/copilot-capability-matrix.json")).toBe(true);
        for (const entry of [
        ...
        expect(selfImprove?.supportFileModes?.["scripts/validate.sh"]).toBe("100755");
        ...
        const npmTarPermissions = tarPermissions(tarballPathCache, `package/${executableSupportPath}`, true);
        expect(isSupportedNpmTarMode(npmTarPermissions)).toBe(true);
        const extractedSupportPath = join(packedPackageFixture.extractedPackageRoot, executableSupportPath);
        const extractedPrompt = readFileSync(join(packedPackageFixture.extractedPackageRoot, "skills-copilot", "self-improve", "SKILL.md"), "utf-8");
        expect(extractedPrompt).toContain('bash "$VALIDATE_SH"');
        expect(extractedPrompt).toContain('VALIDATE_SH="$(cd "{skill_dir}/scripts" && pwd -P)/validate.sh"');
        expect(extractedPrompt).toContain("On Windows, use Git Bash, WSL, or MSYS2 Bash");
        expect(extractedPrompt).not.toContain("./validate.sh");
        const bashValidated = validateWithExplicitBash(extractedSupportPath);
        if (!bashValidated) {
            expect(extractedPrompt.replace(/\s+/g, " ")).toContain("If no Bash runtime is available, stop and report that requirement");
        }
    });
