...
async function waitForPaths(paths, timeoutMs = 5_000) {
...
describe('file-lock', () => {
    ...
    describe('acquireFileLockSync / releaseFileLockSync', () => {
        ...
        it('recovers an authenticated recovery barrier orphaned by a crashed owner', () => {
            ...
            const processUtilsUrl = new URL('../platform/process-utils.ts', import.meta.url).href;
        ...
        it('recovers an orphaned recovery reaper claim without another fixed barrier', () => {
            ...
            const recoveryPath = `${lockPath}.reclaim.guard.recover`;
            const claimPath = `${recoveryPath}.reaper.77777777-7777-4777-8777-777777777777`;
        ...
        it('ignores an incomplete unpublished reaper claim left by a crash', () => {
            ...
            const publicationPath = join(testDir, `.partial-reaper-publication.lock.reclaim.guard.recover.reaper.${nonce}.publish.${nonce}.tmp`);
        ...
        it('reclaims a recovery barrier after authenticated PID reuse', () => {
            ...
            writeFileSync(recoveryPath, JSON.stringify({
                version: 2,
                pid: process.pid,
                processStartIdentity: processStartIdentity === '1' ? '2' : '1',
        ...
        it('only lets the exact recovery barrier owner release its generation', () => {
            const recoveryPath = `${join(testDir, 'owned-recovery-release.lock')}.reclaim.guard.recover`;
        ...
        `;
            ...
            try {
                ...
                await waitForPaths(children.map(({ resultPath }) => resultPath), 7_500);
                const results = children.map(({ resultPath }) => JSON.parse(readFileSync(resultPath, 'utf8')));
