    ...
    describe('validateWorkingDirectory (#576)', () => {
        ...
        describe('installed plugin runtime root (Copilot MCP launch)', () => {
            ...
            it('returns the explicit project git root when cwd is a verified non-git plugin root', () => {
                ...
                expect(errorSpy).toHaveBeenCalledWith('[worktree] plugin MCP server pinned to project root', expect.objectContaining({ projectRoot: expect.any(String) }));
                errorSpy.mockRestore();
            });
            ...
            it('keeps a plugin MCP server pinned to the first accepted project root', () => {
                ...
                try {
                    ...
                    expect(errorSpy).toHaveBeenCalledWith('[worktree] plugin MCP server is already pinned to a different project root', expect.objectContaining({
            ...
            it('rejects when no plugin or host signal corroborates the runtime root', () => {
                ...
                expect(errorSpy).toHaveBeenCalledWith('[worktree] workingDirectory resolved to different git worktree root, using trusted root', expect.any(Object));
                errorSpy.mockRestore();
            });
