...
describe('cli-detection', () => {
    ...
    it('resolves a Windows provider before probing its .cmd shim through COMSPEC', () => {
        ...
        expect(mockSpawnSync).toHaveBeenNthCalledWith(1, 'where', ['codex'], expect.objectContaining({ timeout: 5000 }));
        expect(mockSpawnSync).toHaveBeenNthCalledWith(2, 'C:\\Windows\\System32\\cmd.exe', ['/d', '/c', 'C:\\Tools\\codex.cmd', '--version'], { timeout: 5000 });
    ...
    it('detectAllClis resolves the antigravity binary (agy)', () => {
        ...
        expect(mockSpawnSync).toHaveBeenCalledWith(process.platform === 'win32' ? 'where' : 'which', ['agy'], expect.objectContaining({ timeout: 5000 }));
        clearResolvedPathCache();
        mockSpawnSync.mockRestore();
    });
    it('detects Copilot with `copilot --version` and avoids a Windows shell', () => {
        ...
        clearResolvedPathCache();
        mockSpawnSync
            .mockReturnValueOnce({ status: 0, stdout: 'C:\\Tools\\copilot.exe', stderr: '', pid: 0, output: [], signal: null })
            .mockReturnValueOnce({ status: 0, stdout: 'GitHub Copilot CLI 1.0.71-3', stderr: '', pid: 0, output: [], signal: null });
        ...
        expect(mockSpawnSync).toHaveBeenNthCalledWith(2, 'C:\\Tools\\copilot.exe', ['--version'], { timeout: 5000, shell: false });
        restorePlatform();
        clearResolvedPathCache();
        mockSpawnSync.mockRestore();
    });
    it('detects an npm-installed Copilot .cmd shim through COMSPEC on Windows', () => {
        ...
        expect(mockSpawnSync).toHaveBeenNthCalledWith(2, 'C:\\Windows\\System32\\cmd.exe', ['/d', '/c', 'C:\\Users\\me\\AppData\\Roaming\\npm\\copilot.cmd', '--version'], { timeout: 5000 });
        if (originalComspec === undefined)
            delete process.env.COMSPEC;
        else
            process.env.COMSPEC = originalComspec;
    ...
    it('includes Copilot in all-provider detection', () => {
        ...
        expect(mockSpawnSync).toHaveBeenCalledWith(process.platform === 'win32' ? 'where' : 'which', ['copilot'], expect.objectContaining({ timeout: 5000 }));
        clearResolvedPathCache();
        mockSpawnSync.mockRestore();
    });
