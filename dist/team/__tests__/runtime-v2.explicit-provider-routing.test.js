...
describe('runtime-v2 explicit provider + role preservation', () => {
    ...
    it('preserves an explicit Copilot reviewer provider without native-agent fallback', () => {
        const assignment = resolveTaskAssignment({ subject: 'Review', description: 'review the change', role: 'code-reviewer' }, resolvedRouting, undefined, binaries, 'copilot');
    ...
    it('propagates routed Copilot effort and falls back to the immutable Claude tuple when unavailable', () => {
        ...
        const available = resolveTaskAssignment({ subject: 'Critique', description: 'challenge the plan', role: 'critic' }, routed, { critic: { provider: 'copilot' } }, binaries, 'claude');
        ...
        const unavailable = resolveTaskAssignment({ subject: 'Critique', description: 'challenge the plan', role: 'critic' }, routed, { critic: { provider: 'copilot' } }, { claude: '/usr/bin/claude' }, 'claude');
