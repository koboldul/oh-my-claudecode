...
import { CLAUDE_SINGLE_CAPABILITIES, formatUnknownError, } from './hook-protocol.js';
...
export function sanitizeHookEvaluation(value, fallbackCallId) {
    try {
        if (!isRecord(value)) {
        ...
        if (hasOwn(value, 'mutation') && value.mutation !== undefined) {
            ...
            if (!isRecord(rawMutation)
                ...
                return adapterDenyEvaluation('mutation must contain input and a valid requirement', callId);
            }
            let retryHint;
            if (rawMutation.retryHint !== undefined) {
                if (!isRecord(rawMutation.retryHint)
                    ...
                    return adapterDenyEvaluation('mutation.retryHint must contain a non-empty instruction and optional plain-object patch', callId);
                }
                retryHint = {
...
function aggregateDecision(candidates) {
    if (candidates.some(({ decision }) => decision === 'deny'))
        return 'deny';
    if (candidates.some(({ decision }) => decision === 'ask'))
        return 'ask';
...
function normalizationSafetyIssue(envelope) {
    const batchIssue = envelope.issues.find((issue) => issue.batchSafety || (issue.scope === 'batch' && issue.severity === 'safety'));
    if (batchIssue)
        return batchIssue;
    if (!isPermissionSensitiveEvent(envelope.hookType))
        return undefined;
    return envelope.issues.find((issue) => {
        if (issue.severity !== 'safety')
            return false;
        if (issue.code !== 'malformed-tool-args')
            return true;
        const call = envelope.toolCalls.find((candidate) => candidate.originalIndex === issue.originalIndex
...
function normalizeEffects(evaluations) {
    return evaluations.flatMap((evaluation) => (evaluation.effects ?? []).map((effect) => ({
        ...effect,
...
export function boundHookContexts(messages, maxMessages = MAX_HOOK_CONTEXT_MESSAGES, maxCharacters = MAX_HOOK_CONTEXT_CHARACTERS) {
    ...
    for (const rawMessage of messages) {
        ...
        if (!message || seen.has(message) || bounded.length >= maxMessages)
            continue;
        seen.add(message);
 ...
 */
export function reduceHookEvaluations(envelope, evaluations) {
    try {
...
function reduceHookEvaluationsInternal(envelope, evaluations) {
    const normalizedEvaluations = evaluations.map((evaluation, index) => sanitizeHookEvaluation(evaluation, envelope.toolCalls[index]?.id));
    ...
    const immutableHostDeny = decisionCandidates.find(({ source, decision }) => source === 'host' && decision === 'deny');
    const logicalCallCount = Number.isInteger(envelope.logicalCallCount)
        ? envelope.logicalCallCount
        : envelope.toolCalls.length;
    const emptyPermissionEnvelope = isPermissionSensitiveEvent(envelope.hookType)
        && logicalCallCount === 0;
    let decision = aggregateDecision(decisionCandidates);
    let reason = decision === 'deny'
    ...
    const correlationUnavailable = logicalCallCount > 1
    ...
    if (decision === 'allow'
        ...
        decision = 'pass';
        diagnostics.push('Aggregate allow was reduced to pass because not every logical tool call was explicitly evaluated and allowed.');
    }
    ...
    for (const evaluation of normalizedEvaluations) {
        if (!evaluation.mutation)
            continue;
        const representationIssue = mutationRepresentationIssue(envelope, mutationDecision, evaluation.callId, evaluation.mutation.input);
        ...
        if (evaluation.mutation.requirement === 'required') {
            ...
            const mutationReason = `Required input mutation${callLabel} cannot be represented by ${envelope.contract} because ${representationIssue}; `
                + (retryInstruction
        ...
        diagnostics.push(`Optional input mutation${callLabel} was not applied because ${envelope.contract} cannot represent it: ${representationIssue}; the original input will be used.`);
    }
    ...
    const stagedEffects = allEffects.filter((effect) => effect.commitOn === 'always' || accepted);
    return {
        decision,
        reason,
        retry,
...
export function interpretLegacyOutput(_hookType, output) {
    try {
        if (!isRecord(output)) {
        ...
        let nestedDecision = {};
        if (hasOwn(hookSpecificOutput, 'decision')
        ...
        if (hasOwn(hookSpecificOutput, 'permissionDecision')
        ...
        const hasUpdatedInput = hasNestedUpdatedInput
            || hasHookUpdatedInput
        ...
        const mutationRequirement = hasNestedUpdatedInput || output.mutationRequirement === 'required'
            ? 'required'
            : 'optional';
        const contexts = [
            output.message,
...
function hasBatchSafetyIssue(envelope) {
    return envelope.issues.some((issue) => issue.batchSafety || (issue.scope === 'batch' && issue.severity === 'safety'));
}
 ...
 */
export async function runHookPayload(hookType, raw, processor) {
    let envelope;
    try {
    ...
    if (!hasBatchSafetyIssue(envelope)) {
        ...
        const units = envelope.toolCalls.length === 0
...
export async function runHookJson(hookType, json, processor) {
    let raw;
    try {
        raw = JSON.parse(json);
    }
