...
import { getSkillProtection, getSkillConfig, readSkillActiveState, writeSkillActiveState, clearSkillActiveState, mutateSkillActiveStateLocked, SkillStateCorruptionError, isSkillStateStale, checkSkillActiveState, readSkillActiveStateNormalized, writeSkillActiveStateCopies, upsertWorkflowSkillSlot, markWorkflowSkillCompleted, clearWorkflowSkillSlot, pruneExpiredWorkflowSkillTombstones, resolveAuthoritativeWorkflowSkill, emptySkillActiveStateV2, WORKFLOW_TOMBSTONE_TTL_MS, } from '../index.js';
...
describe('skill-state', () => {
    ...
    describe('writeSkillActiveState', () => {
        ...
        it('returns committed support state after repairing a missing copy', () => {
            ...
            expect(writeSkillActiveState(tempDir, 'plan', 'session-1'))
                .not.toBeNull();
            rmSync(join(tempDir, '.omc', 'state', 'sessions', 'session-1', 'skill-active-state.json'), { force: true });
    ...
    describe('checkSkillActiveState', () => {
        ...
        it('clears stale state and allows stop', () => {
            ...
            const state = readSkillActiveState(tempDir, 'session-1');
            const past = new Date(Date.now() - 10 * 60 * 1000).toISOString();
            expect(mutateSkillActiveStateLocked(tempDir, 'session-1', (current) => ({
                ...current,
                support_skill: {
                    ...state,
                    started_at: past,
    ...
    describe('writeSkillActiveStateCopies — dual-write invariant (spec a/b)', () => {
        ...
        it('honors rootState:null while retaining the session copy', () => {
            ...
            const state = upsertWorkflowSkillSlot(emptySkillActiveStateV2(), 'ralph', { session_id: sessionId });
            expect(writeSkillActiveStateCopies(tempDir, state, sessionId, { rootState: null })).toBe(true);
            expect(existsSync(rootFilePath(tempDir))).toBe(false);
            expect(existsSync(sessionFilePath(tempDir, sessionId))).toBe(true);
            expect(readSkillActiveStateNormalized(tempDir, sessionId).active_skills.ralph).toBeDefined();
        });
        it('honors rootState:null for no-session writes', () => {
            const initial = upsertWorkflowSkillSlot(emptySkillActiveStateV2(), 'autopilot');
            expect(writeSkillActiveStateCopies(tempDir, initial)).toBe(true);
            expect(existsSync(rootFilePath(tempDir))).toBe(true);
            expect(writeSkillActiveStateCopies(tempDir, initial, undefined, { rootState: null })).toBe(true);
            expect(existsSync(rootFilePath(tempDir))).toBe(false);
        });
    });
    // -----------------------------------------------------------------------
    ...
    describe('readSkillActiveStateNormalized — normalization and session authority', () => {
        ...
        it('fails closed without overwriting a corrupt root ledger', () => {
            const rootPath = join(tempDir, '.omc', 'state', 'skill-active-state.json');
            mkdirSync(join(tempDir, '.omc', 'state'), { recursive: true });
            writeFileSync(rootPath, '{corrupt-root');
            const result = mutateSkillActiveStateLocked(tempDir, 'corrupt-root-session', (current) => upsertWorkflowSkillSlot(current, 'ralph', {
        ...
        it('fails closed without replacing a corrupt session ledger', () => {
            ...
            expect(writeSkillActiveStateCopies(tempDir, state, sessionId))
                .toBe(true);
            const sessionPath = join(tempDir, '.omc', 'state', 'sessions', sessionId, 'skill-active-state.json');
            writeFileSync(sessionPath, '{corrupt-session');
            const result = mutateSkillActiveStateLocked(tempDir, sessionId, (current) => upsertWorkflowSkillSlot(current, 'autopilot', {
                session_id: sessionId,
            }));
            expect(result.status).toBe('failed');
        ...
        it('preserves legacy session bytes on a semantic no-op', () => {
            ...
            const result = mutateSkillActiveStateLocked(tempDir, sessionId, current => current);
        ...
        it('migrates legacy session state when a real mutation occurs', () => {
            ...
            const result = mutateSkillActiveStateLocked(tempDir, sessionId, current => ({
                ...current,
