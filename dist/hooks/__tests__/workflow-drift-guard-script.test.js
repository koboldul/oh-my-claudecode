...
const COPILOT_FIXTURE_ROOT = join(ROOT, 'src', '__tests__', 'fixtures', 'hooks', 'copilot-1.0.72-1');
const COPILOT_AGENT_STOP = JSON.parse(readFileSync(join(COPILOT_FIXTURE_ROOT, 'agentStop.json'), 'utf8'));
const COPILOT_TRANSCRIPT_FIXTURE = join(COPILOT_FIXTURE_ROOT, 'agentStop-transcript.jsonl');
...
function copilotGuardInput(transcriptPath, extra = {}) {
    return {
        ...COPILOT_AGENT_STOP,
        cwd: GUARD_CWD,
...
function makeTranscript(contents) {
...
function expectCopilotPass(result) {
