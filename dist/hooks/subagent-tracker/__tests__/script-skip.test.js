...
const loaderPath = resolve(process.cwd(), "scripts/lib/hook-runtime-loader.mjs");
const stdinPath = resolve(process.cwd(), "scripts/lib/stdin.mjs");
const copilotEventFixture = resolve(process.cwd(), "src", "hooks", "subagent-tracker", "__tests__", "fixtures", "copilot-session");
let fixtureRoot;
let fixtureScriptPath;
...
beforeAll(() => {
    ...
    copyFileSync(loaderPath, join(fixtureRoot, "scripts", "lib", "hook-runtime-loader.mjs"));
    copyFileSync(stdinPath, join(fixtureRoot, "scripts", "lib", "stdin.mjs"));
    ...
    buildSync({
        ...
        target: "node20",
        format: "esm",
        outfile: join(fixtureRoot, "dist", "hooks", "subagent-tracker", "index.js"),
        logLevel: "silent",
    });
});
afterAll(() => {
...
function runFixtureTracker(action, payload, env = {}) {
    return new Promise((resolveRun, rejectRun) => {
        const child = spawn(process.execPath, [fixtureScriptPath, action], {
        ...
        child.once("close", (code) => {
            if (code !== 0) {
                rejectRun(new Error(`subagent tracker exited ${String(code)}: ${stderr}`));
                return;
            }
            try {
                resolveRun({
...
function readFixtureState(sessionId) {
    const statePath = join(fixtureWorkspace, ".omc", "state", "sessions", sessionId, "subagent-tracking-state.json");
    return JSON.parse(readFileSync(statePath, "utf8"));
}
function readFixtureMissionState(sessionId) {
    const statePath = join(fixtureWorkspace, ".omc", "state", "sessions", sessionId, "mission-state.json");
    return JSON.parse(readFileSync(statePath, "utf8"));
}
function createFailureFixture(options) {
    ...
    copyFileSync(loaderPath, join(root, "scripts", "lib", "hook-runtime-loader.mjs"));
    copyFileSync(stdinPath, join(root, "scripts", "lib", "stdin.mjs"));
    writeFileSync(join(root, "package.json"), JSON.stringify({ type: "module" }));
    if (options.runtimeSource) {
        writeFileSync(join(root, "bridge", "hook-runtime.cjs"), options.runtimeSource);
    }
    else {
        copyFileSync(join(fixtureRoot, "bridge", "hook-runtime.cjs"), join(root, "bridge", "hook-runtime.cjs"));
    }
    writeFileSync(join(root, "dist", "hooks", "subagent-tracker", "index.js"), options.trackerSource);
    return { root, script, workspace };
}
function runScriptFixture(script, workspace, action, input) {
...
describe("subagent-tracker script skip guard", () => {
    ...
    it("surfaces a visible optional fail-open diagnostic when the bundle is missing", () => {
        ...
        const missingRuntimeScript = join(root, "scripts", "subagent-tracker.mjs");
        mkdirSync(join(root, "scripts", "lib"), { recursive: true });
        copyFileSync(scriptPath, missingRuntimeScript);
        copyFileSync(loaderPath, join(root, "scripts", "lib", "hook-runtime-loader.mjs"));
        copyFileSync(stdinPath, join(root, "scripts", "lib", "stdin.mjs"));
        try {
            const result = spawnSync(process.execPath, [missingRuntimeScript, "start"], {
            ...
            expect(result.stderr).toContain("continuing without optional hook behavior");
        }
        finally {
    ...
    it("fails open visibly when canonical input normalization rejects a stop", () => {
        const result = runScriptFixture(fixtureScriptPath, fixtureWorkspace, "stop", "{");
    ...
    it("fails open visibly when the canonical processor adapter rejects a stop", () => {
      ...
      `,
        ...
        try {
            const result = runScriptFixture(fixture.script, fixture.workspace, "stop", JSON.stringify({
    ...
    it("uses Copilot event-log IDs for subprocess identity and idempotency", async () => {
        ...
        expect(completed.agents.every((agent) => agent.status === "completed")).toBe(true);
    });
    it("preserves all 19 same-name Copilot lifecycles through subprocesses", async () => {
        ...
        let peakRunning = 0;
        const startResults = await Promise.all(Array.from({ length: 19 }, (_, index) => runFixtureTracker("start", {
            sessionId,
            transcriptPath,
            cwd: fixtureWorkspace,
            agentName: "executor",
        ...
        for (const { output, stderr } of startResults) {
            ...
            peakRunning = Math.max(peakRunning, Number(runningMatch?.[1] ?? 0));
        }
        const startedState = readFixtureState(sessionId);
        const startedMission = readFixtureMissionState(sessionId).missions[0];
        expect(new Set(startedState.agents.map((agent) => agent.agent_id)).size).toBe(19);
        expect(startedState.agents.every((agent) => /^[0-9a-f]{12}-[0-9a-f]{12}$/.test(agent.agent_id))).toBe(true);
        ...
        const stopResults = await Promise.all(Array.from({ length: 19 }, (_, index) => runFixtureTracker("stop", {
            sessionId,
            transcriptPath,
            cwd: fixtureWorkspace,
            agentName: "executor",
        ...
        expect(state.agents.filter((agent) => agent.status === "running"))
            .toHaveLength(0);
        expect(state.agents.every((agent) => agent.correlation_strategy === "agent-name-fifo"
            && agent.synthetic_correlation === true)).toBe(true);
