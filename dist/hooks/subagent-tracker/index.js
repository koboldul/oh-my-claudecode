 ...
 */
import { existsSync, readFileSync, writeFileSync, mkdirSync, unlinkSync, readdirSync, openSync, closeSync, readSync, statSync, } from "fs";
...
import { recordAgentReconciliation, recordAgentStart, recordAgentStop, } from './session-replay.js';
...
const REORDERED_LIFECYCLE_TELEMETRY_NOTE = "Subagent lifecycle events arrived out of order and were reconciled deterministically.";
...
function withLifecycleLock(lockPath, processEvent) {
...
export function mergeTrackerStates(diskState, pendingState) {
    ...
    const lifecycle_sequence = Math.max(diskState.lifecycle_sequence ?? 0, pendingState.lifecycle_sequence ?? 0);
    ...
    const delivery_receipts = Array.from(receiptMap.values())
        .sort((left, right) => new Date(left.recorded_at).getTime()
...
function lifecycleLabelMatches(expected, actual) {
    if (!expected)
        return true;
    if (!actual)
        return false;
...
function resolveCopilotEventLogPath(transcriptPath) {
    if (!transcriptPath)
        return undefined;
    const candidates = [
...
function readBoundedEventLogTail(filePath) {
    let fd;
    try {
...
function resolveCopilotLifecycleId(kind, transcriptPath, eventTimestamp, agentName, agentDisplayName) {
    if (eventTimestamp === undefined)
        return undefined;
    const eventLogPath = resolveCopilotEventLogPath(transcriptPath);
    if (!eventLogPath)
        return undefined;
    const expectedType = kind === "start" ? "subagent.started" : "subagent.completed";
    const candidates = [];
    const lines = readBoundedEventLogTail(eventLogPath);
    for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
        let event;
        try {
            event = JSON.parse(lines[lineIndex]);
        }
        catch {
            continue;
        }
        ...
        if (delta > COPILOT_EVENT_MATCH_WINDOW_MS)
            continue;
        const eventAgentName = firstString(event.agentName, data?.agentName);
        const eventDisplayName = firstString(event.displayName, event.agentDisplayName, data?.displayName, data?.agentDisplayName);
        if (!lifecycleLabelMatches(agentName, eventAgentName))
            continue;
        if (!lifecycleLabelMatches(agentDisplayName, eventDisplayName))
            continue;
    ...
    return candidates
        .sort((left, right) => left.delta - right.delta || right.lineIndex - left.lineIndex)[0]?.agentId;
}
function lifecycleDigest(kind, fields) {
    return createHash("sha256")
        .update(JSON.stringify([kind, ...fields]))
        .digest("hex");
}
function digestDisplayId(digest) {
    return `${digest.slice(0, 12)}-${digest.slice(12, 24)}`;
}
function allocateLifecycleIdentity(state, kind, sessionId, contentDigest) {
...
function resolveLifecycleIdentity(state, kind, sessionId, contentDigest, hostAgentId) {
    if (!hostAgentId) {
        return allocateLifecycleIdentity(state, kind, sessionId, contentDigest);
    }
    const identityDigest = lifecycleDigest(kind, [
        sessionId,
        "host",
...
function findDeliveryReceipt(state, action, receipt) {
    if (!receipt)
        return undefined;
    return state.delivery_receipts?.find((entry) => entry.action === action && entry.receipt === receipt);
}
function recordDeliveryReceipt(state, receipt) {
    if (!receipt)
        return;
    state.delivery_receipts = [
        ...(state.delivery_receipts ?? []).filter((entry) => entry.action !== receipt.action || entry.receipt !== receipt.receipt),
        receipt,
    ].slice(-MAX_DELIVERY_RECEIPTS);
}
function updateReceiptAgentId(state, previousAgentId, nextAgentId) {
...
function lifecycleOutput(agent, options = {}) {
    return {
...
function normalizeSubagentStartInput(input) {
    ...
    const agentName = firstString(raw.agent_type, raw.agentName, raw.agent_name);
    const agentDisplayName = firstString(raw.agentDisplayName, raw.agent_display_name);
    const agentDescription = firstString(raw.agentDescription, raw.agent_description);
    if (!explicitAgentId && !agentName && !agentDisplayName)
        return undefined;
    ...
    const eventLogAgentId = !explicitAgentId && raw.host === "copilot"
        ? resolveCopilotLifecycleId("start", transcriptPath, timestamp, agentName, agentDisplayName)
        : undefined;
    ...
    const deliveryReceipt = firstString(raw.deliveryReceipt, raw.delivery_receipt, raw.deliveryId, raw.idempotencyKey, eventPayload?.deliveryReceipt, eventPayload?.deliveryId, eventPayload?.idempotencyKey);
    return {
        session_id: sessionId,
        transcript_path: transcriptPath,
        cwd,
        permission_mode: firstString(raw.permission_mode, raw.permissionMode, eventPayload?.permissionMode) ?? "default",
...
function normalizeSubagentStopInput(input) {
    const raw = input;
    const eventPayload = asRecord(raw.eventPayload);
    const sessionId = firstString(raw.session_id, raw.sessionId);
    const cwd = firstString(raw.cwd, raw.directory);
    ...
    const transcriptPath = firstString(raw.transcript_path, raw.transcriptPath) ?? "";
    const eventLogAgentId = !explicitAgentId && raw.host === "copilot"
        ? resolveCopilotLifecycleId("stop", transcriptPath, timestamp, agentName, agentDisplayName)
        : undefined;
    const agentId = explicitAgentId ?? eventLogAgentId;
    const output = firstString(raw.output, raw.lastAssistantMessage, eventPayload?.lastAssistantMessage, raw.toolOutput, eventPayload?.toolOutput);
    const stopReason = firstString(raw.stop_reason, raw.stopReason);
...
export function processSubagentStart(input) {
    ...
    try {
        const processed = withLifecycleLock(lockPath, () => {
            ...
            const taskDescription = (normalized.prompt
                ?? normalized.agent_description)?.substring(0, 200);
            const rawInput = input;
            const agentName = rawInput?.name?.trim() || undefined;
            const agentDescription = rawInput?.description?.trim() || undefined;
            const existingAgent = state.agents.find((agent) => agent.agent_id === rawInput?.agent_id);
            const isDuplicateRunningStart = existingAgent?.status === "running";
            let trackedAgent;
            const duplicateOutput = (duplicateAgent) => {
                const runningCount = state.agents.filter((agent) => agent.status === "running").length;
                return {
                    continue: true,
                    hookSpecificOutput: {
                        hookEventName: "SubagentStart",
                        additionalContext: `Agent ${duplicateAgent.agent_type} start already recorded `
                            + `(${duplicateAgent.agent_id}); ${runningCount} agent(s) running`,
                        agent_count: runningCount,
                        stale_agents: getStaleAgents(state).map((agent) => agent.agent_id),
                        ...
                        correlation_status: duplicateAgent.correlation_status
                            ?? (duplicateAgent.id_source === "host"
                                ? "host-id"
                                : "unavailable"),
                        synthetic_correlation: duplicateAgent.synthetic_correlation === true,
                        duplicate: true,
                    },
                    tracking: lifecycleOutput(duplicateAgent, { duplicate: true }),
                };
            };
            const priorReceipt = findDeliveryReceipt(state, "start", normalized.delivery_receipt);
            if (priorReceipt) {
                const duplicateAgent = priorReceipt.agent_id
                    ? state.agents.find((agent) => agent.agent_id === priorReceipt.agent_id)
                    : undefined;
                return {
                    output: duplicateAgent
                        ...
                        : {
                            ...
                            hookSpecificOutput: {
                                hookEventName: "SubagentStart",
                                additionalContext: `Subagent start delivery ${priorReceipt.receipt} already recorded`,
                                duplicate: true,
                            },
                        },
                    record: undefined,
            ...
            let existingIndex = normalized.agent_id
                ? state.agents.findIndex((agent) => agent.agent_id === normalized.agent_id)
                : -1;
            if (existingIndex !== -1) {
                const existingAgent = state.agents[existingIndex];
                const pendingReordered = existingAgent.status !== "running"
                    && existingAgent.telemetry_status === "unmatched_stop"
                    && existingAgent.start_event_id === undefined;
                if (pendingReordered
                    && !canReconcileReorderedStop(existingAgent, normalized.event_timestamp)) {
                    const previousAgentId = existingAgent.agent_id;
                    existingAgent.reported_agent_id ??= previousAgentId;
                    existingAgent.agent_id = digestDisplayId(existingAgent.stop_identity_digest
                    ...
                    updateReceiptAgentId(state, previousAgentId, existingAgent.agent_id);
                    existingIndex = -1;
                }
                    ...
                    existingAgent.synthetic_correlation ??=
                        normalized.id_source === "synthetic";
                    recordDeliveryReceipt(state, normalized.delivery_receipt
                        ? {
                            action: "start",
            ...
            const identity = resolveLifecycleIdentity(state, "start", normalized.session_id, normalized.content_digest, normalized.agent_id);
            const agentId = normalized.agent_id
                ?? digestDisplayId(identity.identityDigest);
            if (existingIndex === -1
                && normalized.agent_name) {
                existingIndex = findPendingReorderedStopByName(state, normalized.agent_name, normalized.event_timestamp, normalized.agent_id);
            }
            let reordered = false;
            let previousAgentId;
            if (existingIndex !== -1) {
                ...
                const hasHostIdentity = normalized.id_source === "host"
                ...
                if (existingAgent.completed_at) {
                    existingAgent.duration_ms = Math.max(0, new Date(existingAgent.completed_at).getTime()
                ...
                const agentInfo = {
                    ...
                    correlation_status: normalized.id_source === "host" ? "host-id" : "unavailable",
                    correlation_strategy: normalized.id_source === "synthetic"
                        ? "synthetic-start-id"
                        : "host-id",
            ...
            return {
                output: {
                    ...
                    hookSpecificOutput: {
                        hookEventName: "SubagentStart",
                        additionalContext: `Agent ${trackedAgent.agent_type} started `
                        ...
                        correlation_status: trackedAgent.correlation_status
                            ?? (trackedAgent.id_source === "host"
                                ? "host-id"
                                : "unavailable"),
                        synthetic_correlation: trackedAgent.synthetic_correlation === true,
        ...
        if (processed.record) {
            const { trackedAgent, parentMode, reordered, startedAt, previousAgentId, } = processed.record;
            try {
                recordAgentStart(normalized.cwd, normalized.session_id, trackedAgent.agent_id, trackedAgent.agent_type, normalized.prompt ?? normalized.agent_description, parentMode, normalized.model);
            }
            catch { /* best-effort */ }
            if (reordered && trackedAgent.status !== "running") {
                try {
                    recordAgentReconciliation(normalized.cwd, normalized.session_id, previousAgentId ?? trackedAgent.agent_id, trackedAgent.agent_id, trackedAgent.agent_type, trackedAgent.status === "completed", trackedAgent.duration_ms);
                }
                catch { /* best-effort */ }
            }
            try {
                recordMissionAgentStart(normalized.cwd, {
                    ...
                    parentMode,
                    taskDescription: normalized.prompt ?? normalized.agent_description,
 ...
 */
function findRunningAgentByNameFifo(state, agentName) {
    return state.agents
        ...
        const rightSequence = right.agent.start_sequence ?? Number.MAX_SAFE_INTEGER;
        if (leftSequence !== rightSequence) {
            return leftSequence - rightSequence;
        }
        const timeDifference = new Date(left.agent.started_at).getTime()
            - new Date(right.agent.started_at).getTime();
        return timeDifference || left.index - right.index;
    })[0]?.index ?? -1;
}
function canReconcileReorderedStop(agent, startTimestamp) {
...
function findPendingReorderedStopByName(state, agentName, startTimestamp, startAgentId) {
    return state.agents
        .map((agent, index) => ({ agent, index }))
        .filter(({ agent }) => agent.status !== "running"
        ...
        .sort((left, right) => {
        const timeDifference = new Date(left.agent.completed_at ?? left.agent.started_at).getTime()
...
export function processSubagentStop(input) {
    ...
    try {
        const processed = withLifecycleLock(lockPath, () => {
            ...
            const priorReceipt = findDeliveryReceipt(state, "stop", normalized.delivery_receipt);
            if (priorReceipt) {
                const duplicateAgent = priorReceipt.agent_id
                    ? state.agents.find((agent) => agent.agent_id === priorReceipt.agent_id)
                    : undefined;
            ...
            const historicalExactDuplicate = normalized.agent_id
                ? state.agents.find((agent) => agent.status !== "running"
            ...
            if (historicalExactDuplicate) {
                return {
                    output: {
                        continue: true,
                        suppressOutput: true,
                        tracking: lifecycleOutput(historicalExactDuplicate, { duplicate: true }),
                    },
                    record: undefined,
                };
            }
            let agentIndex = normalized.agent_id
                ? state.agents.findIndex((agent) => agent.agent_id === normalized.agent_id)
                : -1;
            let correlationStrategy = "host-id";
            if (agentIndex === -1
                && !normalized.agent_id
                && normalized.agent_name) {
                agentIndex = findRunningAgentByNameFifo(state, normalized.agent_name);
            ...
            if (agentIndex !== -1) {
                ...
                const stopIdentity = resolveLifecycleIdentity(state, "stop", normalized.session_id, normalized.content_digest, normalized.agent_id);
                ...
                agent.synthetic_correlation =
                    correlationStrategy === "agent-name-fifo";
                const startTime = new Date(agent.started_at).getTime();
                agent.duration_ms = Math.max(0, new Date(nowIso).getTime() - startTime);
                ...
                const synthetic = {
                    ...
                    host_id_source: normalized.host_id_source,
                    correlation_status: normalized.agent_id ? "host-id" : "unavailable",
            ...
            recordDeliveryReceipt(state, stoppedAgent && normalized.delivery_receipt
                ? {
                    action: "stop",
                    receipt: normalized.delivery_receipt,
                    fingerprint: normalized.delivery_fingerprint,
        ...
        if (processed.record) {
            ...
            try {
                ...
                const stopMetadata = evidence && evidence.kind === "dirty"
                    ...
                    : baseStopMetadata;
                recordAgentStop(normalized.cwd, normalized.session_id, stoppedAgent.agent_id, stoppedAgent.agent_type, succeeded, stoppedAgent.duration_ms, stopMetadata);
            }
            catch { /* best-effort */ }
            if (!stoppedAgent.synthetic) {
                try {
                    recordMissionAgentStop(normalized.cwd, {
                        ...
                        success: succeeded,
                        outputSummary: stoppedAgent.output_summary ?? normalized.output,
