...
function finalizeLegacyPresentation(plan, reduction, commitReport) {
    ...
    if (solePresentation.kind !== 'context'
        || !solePresentation.advisoryIntentId) {
        return solePresentation;
    }
    const claim = commitReport.advisoryClaims[solePresentation.advisoryIntentId]
        ?? 'indeterminate';
 ...
 */
export function finalizePreToolReduction(plan, reduction, commitReport) {
    ...
    return {
        ...
        legacyPresentation: plan.envelope.host === 'claude'
 ...
 */
export function encodePreToolEnforcerOutput(envelope, finalized) {
