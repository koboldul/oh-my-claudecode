...
import { closeSync, existsSync, fstatSync, lstatSync, openSync, readFileSync, readSync, readdirSync, } from 'node:fs';
import { homedir } from 'node:os';
import { basename, join } from 'node:path';
import { getOmcRoot } from '../../lib/worktree-paths.js';
import { parseJsonc } from '../../utils/jsonc.js';
import { PRE_TOOL_MAX_FUTURE_SKEW_MS, PRE_TOOL_MIN_EPOCH_MS, PRE_TOOL_SNAPSHOT_VERSION, } from './types.js';
...
function defaultReadText(path, maxBytes = Number.POSITIVE_INFINITY) {
    let linkStat;
    try {
...
function defaultReadTextTail(path, maxBytes) {
    let linkStat;
    try {
        linkStat = lstatSync(path);
    }
...
function defaultListDirectories(path) {
    try {
...
function loadJsoncRecord(path, readText) {
...
function extractTracking(value) {
    ...
    return {
        running: agents.filter((agent) => isRecord(agent) && agent.status === 'running').length,
...
function readOwnedState(stateDir, sessionId, fileName, readJson, allowLegacyFallback = false) {
    ...
    for (const path of paths) {
        ...
        if (isLegacyFallback && owner !== sessionId)
            continue;
...
function readCanonicalTeam(stateDir, sessionId, readJson, listDirectories) {
    ...
    for (const teamName of listDirectories(teamRoot)) {
        ...
        if (stringValue(leader?.session_id) !== sessionId)
            continue;
...
function isUltragoalTerminal(state, plan) {
    ...
    if (phase && ULTRAGOAL_TERMINAL_PHASES.has(phase))
        return true;
    const aggregate = asRecord(plan?.aggregateCompletion);
...
function extractGoalFromTranscript(transcript, transcriptPath, sessionId) {
    ...
    for (const line of transcript.split('\n')) {
        if (!line || !GOAL_BEARING_HINT.test(line))
            continue;
        let entry;
        try {
...
function estimateContextPercent(tail) {
    ...
    if (!windowMatches || !inputMatches)
        return 0;
    const contextWindow = Number.parseInt(windowMatches.at(-1)?.match(/(\d+)/)?.[1] ?? '0', 10);
    const inputTokens = Number.parseInt(inputMatches.at(-1)?.match(/(\d+)/)?.[1] ?? '0', 10);
...
function parseForceDelegationConfig(value) {
    ...
    if (!raw || raw.enforce !== true || !Array.isArray(raw.rules))
        return null;
    return {
        enforce: true,
        rules: raw.rules
...
function parseForceDelegationEvents(value, nowSec) {
    ...
    return events.flatMap((event, originalIndex) => {
        ...
        if (!Number.isSafeInteger(observedAtSec)
...
function parseAdvisoryEntries(value, nowMs) {
    ...
    return Object.fromEntries(Object.entries(entries).flatMap(([key, entry]) => {
        ...
        if (typeof lastEmittedAtMs !== 'number'
 ...
 */
export function loadPreToolBatchSnapshot(envelope, dependencies = {}) {
    ...
    const readTextTail = (path, maxBytes) => {
        const key = `${path}\0${maxBytes}`;
    ...
    const userOmcConfigPath = join(resolveClaudeConfigDirectory(environment), '.omc-config.json');
    const projectOmcConfigPath = join(omcRoot, 'config.json');
    const omcConfig = asRecord(readJson(userOmcConfigPath))
        ?? asRecord(readJson(projectOmcConfigPath))
        ?? {};
    const userConfig = loadJsoncRecord(join(resolveUserConfigDirectory(environment), 'claude-omc', 'config.jsonc'), readText);
    const projectConfig = loadJsoncRecord(join(directory, '.claude', 'omc.jsonc'), readText);
    const agentTypes = [...new Set(envelope.toolCalls
            .map((call) => getAgentType(call.input))
            .filter(Boolean)
            .map(normalizeAgentType))];
    const configuredAgentModels = Object.fromEntries(agentTypes.map((agentType) => [
        agentType,
        resolveConfiguredAgentModel(agentType, projectConfig, userConfig),
    ]));
    const agentDefinitionModels = Object.fromEntries(agentTypes.map((agentType) => [
        agentType,
        readAgentDefinitionModel(agentType, environment, runtimeDirectory, readText),
    ]));
    const configuredCopilotModel = resolveConfiguredCopilotDefault('copilotModel', projectConfig, userConfig);
    const configuredCopilotEffort = stringValue(environment.OMC_COPILOT_REASONING_EFFORT) || resolveConfiguredCopilotDefault('copilotReasoningEffort', projectConfig, userConfig) || COPILOT_DEFAULT_REASONING_EFFORT;
    const normalizedCopilotEffort = configuredCopilotEffort.toLowerCase();
    const copilotEffortValid = COPILOT_REASONING_EFFORTS.has(normalizedCopilotEffort);
    const copilotDefaults = {
        model: stringValue(environment.OMC_EXTERNAL_MODELS_DEFAULT_COPILOT_MODEL)
    ...
    const modeStates = Object.fromEntries(MODE_STATE_FILES.map((fileName) => [
        fileName.replace(/-state\.json$/, ''),
        readOwnedState(stateDir, sessionId, fileName, readJson, fileName === 'ultragoal-state.json'
            || fileName === 'team-state.json'),
    ]));
    const invalidSessionId = sessionId.length > 0 && !SESSION_ID_PATTERN.test(sessionId);
    const swarmSummaryPath = sessionStatePath(stateDir, sessionId, 'swarm-summary.json') ?? (invalidSessionId ? null : join(stateDir, 'swarm-summary.json'));
    const swarmMarkerPath = sessionStatePath(stateDir, sessionId, 'swarm-active.marker') ?? (invalidSessionId ? null : join(stateDir, 'swarm-active.marker'));
    const swarmSummary = swarmSummaryPath
        ? asRecord(readJson(swarmSummaryPath))
        : null;
    const modeActive = Object.values(modeStates).some((candidate) => hasActiveModeState(candidate.state))
        || (swarmMarkerPath !== null
            && fileExists(swarmMarkerPath)
            && swarmSummary?.active === true);
    const coarseTeamState = modeStates.team?.state ?? null;
    const canonicalTeam = readCanonicalTeam(stateDir, sessionId, readJson, listDirectories);
    const team = coarseTeamState?.active === true
        ? {
            active: true,
            teamName: stringValue(coarseTeamState.team_name)
    ...
    const todoLabel = todoCounts.pending + todoCounts.inProgress > 0
        ...
        : '';
    const tracking = extractTracking(readJson(join(stateDir, 'subagent-tracking.json')));
    ...
    const transcriptTail = transcriptPath
        ...
        : '';
    const needsTranscriptGoal = !!transcriptPath
    ...
    const contextThreshold = numberFromEnvironment(environment.OMC_AGENT_PREFLIGHT_CONTEXT_THRESHOLD, DEFAULT_CONTEXT_THRESHOLD, 1, 100);
    const ultragoalState = modeStates.ultragoal?.state ?? null;
    const ultragoalPlan = asRecord(readJson(join(omcRoot, 'ultragoal', 'goals.json')))
        ?? asRecord(readJson(join(directory, '.omc', 'ultragoal', 'goals.json')));
    const forceDelegation = parseForceDelegationConfig(omcConfig);
    const forceDelegationState = asRecord(readJson(join(stateDir, 'force-agent-delegation-events.json')));
    const forceDelegationGeneration = typeof forceDelegationState?.generation === 'number'
    ...
    const forceDelegationLedger = {
        ...
        events: parseForceDelegationEvents(forceDelegationState, observedAtSec),
    };
    const advisoryPath = sessionId && !SESSION_ID_PATTERN.test(sessionId)
        ? ''
        : SESSION_ID_PATTERN.test(sessionId)
            ? join(stateDir, 'sessions', sessionId, 'pre-tool-advisory-throttle.json')
            : join(stateDir, 'pre-tool-advisory-throttle.json');
    const advisoryCooldownMs = numberFromEnvironment(environment.OMC_PRE_TOOL_ADVISORY_COOLDOWN_MS, DEFAULT_ADVISORY_COOLDOWN_MS, 0);
    const advisoryNowMs = timestampFromEnvironment(environment.OMC_PRE_TOOL_ADVISORY_NOW_MS, loadedAtMs);
    ...
    const snapshot = {
        ...
        deliveryId,
        environment,
        disabled: environment.DISABLE_OMC === '1'
        ...
        omcConfig,
        modelRouting: {
            forceInherit: environment.OMC_ROUTING_FORCE_INHERIT === 'true'
            ...
            copilotDefaults,
            tierEnvironment: Object.fromEntries(Object.entries(environment)
        ...
        ultragoal: {
            ...
            ...(modeStates.ultragoal?.path
                ...
                : {}),
            plan: ultragoalPlan,
            expectedObjective: expectedUltragoalObjective(ultragoalState, ultragoalPlan),
            terminal: isUltragoalTerminal(ultragoalState, ultragoalPlan),
            ...(transcriptGoal ? { goal: transcriptGoal } : {}),
        },
        forceDelegation,
