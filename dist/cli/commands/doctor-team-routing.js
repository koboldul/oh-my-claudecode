    ...
    if (options.json) {
        ...
        for (const p of probes) {
            if (p.runnable) {
                ...
                console.log(`  ${colors.yellow('⚠')} ${p.provider}: resolved${resolvedPath}, but the version probe failed (version unavailable)${detail} — fix the provider before routing /team tasks to it`);
            }
            else {
