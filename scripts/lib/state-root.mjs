 ...
 * `<pluginRoot>/scripts/lib/state-root.mjs`. That candidate is only trusted
 * when it looks like the real OMC package root (has package.json AND
 * .claude-plugin/plugin.json), so a standalone copy of this file (e.g.
 * ~/.claude/hooks/lib/state-root.mjs) never picks up an unrelated dist/.
 *
