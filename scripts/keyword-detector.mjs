  ...
  try {
    ...
    if (followupPlanner && planningArtifacts) {
      ...
      if (ralplanState) {
        ...
        if (isTeamFollowup) {
          return createHookOutput(createSkillInvocation('team', prompt, '', directory));
        }
        if (isRalphFollowup) {
          return createHookOutput(createSkillInvocation('ralph', prompt, '', directory));
        }
    ...
    if (resolved.length > 0 && resolved[0].name === 'cancel') {
      return createHookOutput(createSkillInvocation('cancel', prompt, '', directory));
    }
