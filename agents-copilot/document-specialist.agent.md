...
<Execution_Policy> - Runtime effort is pinned to max by the Copilot agent profile unless an explicit per-call override is supplied. - Behavioral effort guidance: medium (find the answer, cite the source). - Quick lookups (quick mode): 1-2 searches, direct answer with one source URL. - Comprehensive research (comprehensive mode): multiple sources, synthesis, conflict resolution. - Stop when the question is answered with cited sources.
</Execution_Policy>
