...
describe('omc ask command', () => {
  ...
  it('blocks copilot when disableExternalLLM is enabled by strict security mode', () => {
    ...
    try {
      const stubPath = writeAdvisorStub(wd);
      const result = runCli(
        ['ask', 'copilot', 'review this'],
        wd,
        { OMC_ASK_ADVISOR_SCRIPT: stubPath, OMC_SECURITY: 'strict' },
      );
...
describe('run-provider-advisor script contract', () => {
  ...
  ] as const)('strips parent host markers while preserving auth for %s advisor spawns', (provider, args) => {
  ...
  it('launches copilot headlessly with deterministic autonomous argv and no Windows shell/stdin', () => {
    ...
    try {
      ...
      const calls = JSON.parse(readFileSync(capturePath, 'utf8')) as Array<{
        command: string;
        args: string[];
        options: { shell: boolean; input: string | null; stdio: unknown };
      }>;
      expect(calls).toHaveLength(2);
  ...
  it('applies canonical Copilot model and effort overrides to advisor argv', () => {
    ...
    try {
      ...
      const result = runAdvisorScriptWithPrelude(
        preludePath,
        ['copilot', '--prompt', 'review overrides'],
        wd,
        {
          SPAWN_CAPTURE_PATH: capturePath,
  ...
  it.runIf(process.platform === 'win32')('preserves quoted prompts through the PowerShell 7 npm shim', () => {
    ...
    try {
      ...
      const result = spawnSync(
        'pwsh.exe',
        ['-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-File', ps1Shim, '-p', prompt],
        { encoding: 'utf8' },
      );
