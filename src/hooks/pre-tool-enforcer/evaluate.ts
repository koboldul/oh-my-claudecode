...
import {
  ...
  type PreToolBatchPlan,
  type PreToolBatchSnapshot,
  type PreToolCallPlan,
  ...
  type VirtualForceDelegationLedger,
} from './types.js';
...
const TIER_TO_DEFAULT_ENV_KEYS: Readonly<Record<string, readonly string[]>> = {
  haiku: [
    'OMC_SUBAGENT_MODEL',
...
const SKILL_PROTECTION: Readonly<Record<
  string,
...
const SLOP_DOC_CONTEXT_PATTERN =
  /(?:^|[/\\])(?:docs?|documentation|guides?|instructions?|prompts?|\.om[ctx])(?:[/\\]|$)|\.(?:md|mdx|txt|rst)$/i;
const SLOP_SELF_REFERENCE_PATH_PATTERN =
  /(?:^|[/\\])(?:pre-tool-enforcer(?:\.mjs)?|pre-tool-enforcer\.test\.ts)(?:$|[/\\])/i;
...
function getAgentType(input: Record<string, unknown>): string {
...
function hookEffect(
  call: CanonicalToolCall,
  type: PreToolHookEffect['type'],
  payload: PreToolHookEffect['payload'],
  commitOn: PreToolHookEffect['commitOn'],
  critical = false,
): PreToolHookEffect {
  return {
    type,
...
function extractSkill(input: unknown): {
  skillName: string;
  rawSkillName: string;
} | null {
...
function isBundledOmcSubagent(agentType: string): boolean {
  return /^oh-my-claudecode:[a-zA-Z0-9_-]+$/.test(agentType);
}

function isProviderSpecificModelId(modelId: string): boolean {
...
function hasExtendedContextSuffix(modelId: string): boolean {
  return /\[\d+[mk]\]$/i.test(modelId);
}

function isSubagentSafeModelId(modelId: string): boolean {
  return isProviderSpecificModelId(modelId)
    && !hasExtendedContextSuffix(modelId);
}

function isTierAlias(modelId: string): boolean {
  return TIER_ALIASES.has(modelId.toLowerCase());
}

function normalizeToCcAlias(model: string): string | null {
...
function isBedrockProvider(snapshot: PreToolBatchSnapshot): boolean {
...
function isVertexProvider(snapshot: PreToolBatchSnapshot): boolean {
...
function isNonClaudeProvider(snapshot: PreToolBatchSnapshot): boolean {
  ...
  if (
    snapshot.modelRouting.anthropicBaseUrl
...
function resolveTierAliasToSafeModel(
  tierAlias: string,
  snapshot: PreToolBatchSnapshot,
): string {
  ...
  for (const key of keys) {
    ...
    const isNativeClaudeCode =
      isAnthropicDefault || key.startsWith('CLAUDE_CODE_BEDROCK_');
 ...
 */
export function evaluateModelRouting(
  call: CanonicalToolCall,
  envelope: CanonicalHookEnvelope,
  snapshot: PreToolBatchSnapshot,
): ModelRoutingEvaluation {
  if (
    (call.canonicalName !== 'Task' && call.canonicalName !== 'Agent')
    || !isRecord(call.input)
  ) {
    return { warning: '' };
  }
  ...
  if (envelope.host === 'copilot' && isBundledOmcSubagent(agentType)) {
    ...
    const hasExplicitReasoning =
      (
        Object.hasOwn(input, 'reasoning_effort')
  ...
  if (snapshot.modelRouting.forceInherit) {
    ...
    const sessionHasLmSuffix =
      hasExtendedContextSuffix(claudeModel)
    ...
    if (toolModel) {
      if (
        ...
        return {
          warning: '',
          denyReason:
            `[MODEL ROUTING] This environment uses a non-standard provider (Bedrock/Vertex/proxy). ${guidance} `
      ...
      return {
        warning: '',
        denyReason:
          `[MODEL ROUTING] Your session model "${sessionModel}" has a context-window suffix ([1m]) that sub-agents cannot inherit — `
    ...
    if (!toolModel && nonEmptyString(input.subagent_type)) {
      const canonicalAgent = normalizeAgentType(
        nonEmptyString(input.subagent_type),
      );
      const definitionModel =
        snapshot.modelRouting.agentDefinitionModels[canonicalAgent];
      ...
      if (
        ...
        return {
          warning: '',
          denyReason:
            `[MODEL ROUTING] Agent type "${canonicalAgent}" has model "${definitionModel}" in its definition, `
  ...
  if (!toolModel && agentType) {
    ...
    const configured =
      snapshot.modelRouting.configuredAgentModels[canonicalAgent];
...
function observedModeGeneration(
...
function observedModeOwnerSessionId(
  state: Readonly<Record<string, unknown>> | null,
): string {
  const meta = state && isRecord(state._meta) ? state._meta : null;
...
function observedModeConfirmationTimestamp(
  state: Readonly<Record<string, unknown>> | null,
): string {
  return state
...
function observedModeStateDigest(
  state: Readonly<Record<string, unknown>> | null,
): string {
  return createHash('sha256')
...
function isCancelBootstrap(call: CanonicalToolCall): boolean {
  const input = isRecord(call.input) ? call.input : {};
  if (
    call.canonicalName === 'Skill'
  ...
  if (call.canonicalName === 'Read') {
    const filePath =
      nonEmptyString(input.file_path) || nonEmptyString(input.path);
    const normalized = filePath.replace(/\\/g, '/');
    if (
      /(?:^|\/)(?:skills|skill-bodies)\/cancel\/SKILL\.md$/i.test(normalized)
    ) {
      return true;
    }
  }
  ...
  return isSingleShellCommand(command)
    && /^(?:omc|oh-my-claudecode|gjc)\s+(?:state\s+(?:clear|read|write|list-active|get-status)|cancel)\b/.test(
      command,
    );
}

function isUltragoalBootstrap(call: CanonicalToolCall): boolean {
  const input = isRecord(call.input) ? call.input : {};
  if (
    call.canonicalName === 'Skill'
  ...
  const command = nonEmptyString(input.command);
  return isSingleShellCommand(command)
    && /^(?:omc|oh-my-claudecode)\s+ultragoal\s+(?:create(?:-goals)?|complete(?:-goals)?|next|start-next|status|checkpoint|record-review-blockers)\b/.test(
      command,
    );
}
 ...
 */
export function evaluateUltragoal(
  call: CanonicalToolCall,
  snapshot: PreToolBatchSnapshot,
): string | undefined {
  ...
  if (isUltragoalBootstrap(call) || isCancelBootstrap(call)) return undefined;

  const state = snapshot.ultragoal.state;
  if (!state || state.active !== true) return undefined;
  if (isStaleState(state, snapshot.loadedAtMs)) return undefined;
  ...
  if (snapshot.ultragoal.terminal) return undefined;
  if (isAwaitingConfirmation(state, snapshot.loadedAtMs)) return undefined;
  ...
  const activeStatus =
    ...
    || status === 'in_progress'
    || status === 'running';
  if (!expectedObjective && actualObjective && activeStatus) return undefined;
  if (
    actualObjective
    && expectedObjective
...
function patternMatches(pattern: string | undefined, toolName: string): boolean {
...
function forceDelegationReason(
  rule: ForceDelegationRule,
  observed: number,
  toolName: string,
 ...
 */
export function evaluateForceDelegationPure(
  call: CanonicalToolCall,
  snapshot: PreToolBatchSnapshot,
  ledger: VirtualForceDelegationLedger,
): ForceDelegationEvaluation {
  ...
  const cutoff =
    snapshot.observedAtSec - FORCE_DELEGATION_RETENTION_SECONDS;
  ...
  const currentEvent: VirtualForceDelegationEvent = {
  ...
  const nextLedger: VirtualForceDelegationLedger = {
    events,
  };
  ...
  for (const rule of config.rules) {
    ...
    const observed = nextLedger.events.filter(
      (event) =>
        event.observedAtSec > windowCutoff
        && patternMatches(rule.pattern, event.toolName),
    ).length;
    if (observed >= count) {
      return {
        nextLedger,
...
function collectLikelyPathValues(
  value: unknown,
  output: string[] = [],
  depth = 0,
): string[] {
  if (depth > 5 || output.length > 100 || !isRecord(value)) return output;
  for (const [key, child] of Object.entries(value)) {
    if (
      typeof child === 'string'
    ...
    if (Array.isArray(child)) {
      for (const item of child) {
        if (isRecord(item)) collectLikelyPathValues(item, output, depth + 1);
      }
...
function stripSlopQuotedAndCodeContexts(text: string): string {
  return text
    .replace(/```[\s\S]*?```/g, '\n')
    .replace(/`[^`\r\n]*`/g, ' ')
    .replace(/(["'])(?:\\.|(?!\1)[^\\\r\n])*\1/g, ' ');
...
function removeBenignSlopSpans(text: string): string {
  ...
  const hasAction = stripSlopQuotedAndCodeContexts(inspectedText)
    ...
    .some((segment) =>
      SLOP_FALLBACK_ACTION_PATTERNS.some((pattern) =>
        pattern.test(removeBenignSlopSpans(segment)),
      ),
    );
  if (!hasAction) return '';
...
function generateToolMessage(
  toolName: string,
  snapshot: PreToolBatchSnapshot,
): string {
  if (
    snapshot.quietLevel >= 1
  ...
  const messages: Readonly<Record<string, string>> = {
    TodoWrite:
      `${prefix}Mark todos in_progress BEFORE starting, completed IMMEDIATELY after finishing.`,
    Bash:
      `${prefix}Use parallel execution for independent tasks. Use run_in_background for long operations (npm install, builds, tests).`,
    Edit:
      `${prefix}Verify changes work after editing. Test functionality before marking complete.`,
    Write:
      `${prefix}Verify changes work after editing. Test functionality before marking complete.`,
    Read:
      `${prefix}Read multiple files in parallel when possible for faster analysis.`,
    Grep:
      `${prefix}Combine searches in parallel when investigating multiple patterns.`,
    Glob:
      `${prefix}Combine searches in parallel when investigating multiple patterns.`,
  };
...
function combineMessages(...messages: readonly string[]): string {
  return messages.filter(Boolean).join('\n\n');
}

function extractAskUserQuestion(input: unknown): string {
  if (!isRecord(input) || !Array.isArray(input.questions)) {
    return 'User input requested';
  }
  const questions = input.questions
    .map((question) =>
      isRecord(question) ? nonEmptyString(question.question) : '',
    )
    .filter(Boolean);
 ...
 */
export function buildAdvisoryCandidate(
  call: CanonicalToolCall,
  snapshot: PreToolBatchSnapshot,
  message: string,
...
function buildPreflightReason(snapshot: PreToolBatchSnapshot): string {
...
function deniedCallPlan(
  call: CanonicalToolCall,
  reason: string,
  effects: readonly PreToolHookEffect[],
  nextLedger: VirtualForceDelegationLedger,
  presentationKind: 'hook-deny' | 'raw-block' = 'hook-deny',
  mutation?: HookMutation,
): PreToolCallPlan {
  const evaluation: HookEvaluation = {
    callId: call.id,
    source: 'handler',
    decision: 'deny',
    reason,
...
export function evaluatePreToolCall(
  ...
  const effects: PreToolHookEffect[] = [];
  if (call.canonicalName === 'Skill') {
    ...
    if (skill) {
      if (snapshot.directory && snapshot.sessionId) {
        ...
        const payload: TraceSkillAttemptEffectPayload = {
          version: PRE_TOOL_EFFECT_PAYLOAD_VERSION,
          intentId,
          originalIndex: call.originalIndex,
      ...
      if (protection !== 'none') {
        ...
        const payload: SupportSkillUpsertEffectPayload = {
          version: PRE_TOOL_EFFECT_PAYLOAD_VERSION,
          intentId,
          originalIndex: call.originalIndex,
          directory: snapshot.directory,
      ...
      for (
        const modeName of
        MODE_CONFIRMATION_SKILL_MAP[skill.skillName] ?? []
      ) {
        ...
        const payload: ModeConfirmEffectPayload = {
          version: PRE_TOOL_EFFECT_PAYLOAD_VERSION,
          intentId,
          originalIndex: call.originalIndex,
          directory: snapshot.directory,
          ...
          observedOwnerSessionId:
            observedModeOwnerSessionId(observation.state),
          observedGeneration: observedModeGeneration(observation.state),
          observedConfirmationTimestamp:
            observedModeConfirmationTimestamp(observation.state),
    ...
    ? requiredMutation(
        ...
        Object.fromEntries(
          Object.entries(modelRouting.updatedInput).filter(
            ([key, value]) =>
              !isRecord(call.input) || call.input[key] !== value,
          ),
        ),
      )
    : undefined;
  ...
  if (call.canonicalName === 'AskUserQuestion') {
    ...
    const payload: AskUserNotifyEffectPayload = {
      version: PRE_TOOL_EFFECT_PAYLOAD_VERSION,
      intentId,
      originalIndex: call.originalIndex,
      directory: snapshot.directory,
  ...
  if (
    AGENT_HEAVY_TOOLS.has(call.canonicalName)
  ...
  let advisoryCandidate: AdvisoryCandidate | undefined;
  ...
  let legacyPresentation: LegacyPresentation;
 ...
 */
export function planPreToolBatch(
  envelope: CanonicalHookEnvelope,
  snapshot: PreToolBatchSnapshot,
): PreToolBatchPlan {
  ...
  const seenAdvisoryHashes = new Set<string>();
  const calls: PreToolCallPlan[] = [];
  for (
    const call of [...envelope.toolCalls]
    ...
    if (advisory && seenAdvisoryHashes.has(advisory.messageHash)) {
      ...
      const updatedInput =
        plan.legacyPresentation.kind === 'context'
