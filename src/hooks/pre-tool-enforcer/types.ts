...
export const PRE_TOOL_SNAPSHOT_VERSION = 1 as const;
export const PRE_TOOL_EFFECT_PAYLOAD_VERSION = 1 as const;
export const PRE_TOOL_MAX_FUTURE_SKEW_MS = 5 * 60 * 1000;
export const PRE_TOOL_MIN_EPOCH_MS = Date.UTC(2000, 0, 1);

export type PreToolFinalDisposition = 'accepted' | 'rejected';
export type AdvisoryClaimDisposition =
  | 'granted'
  | 'throttled'
  | 'indeterminate';
...
export interface TraceSkillAttemptEffectPayload
  extends PreToolEffectPayloadBase {
  readonly directory: string;
  readonly sessionId: string;
...
export interface ForceDelegationAttemptEffectPayload
  extends PreToolEffectPayloadBase {
  readonly stateDir: string;
  readonly toolName: string;
  readonly observedAtSec: number;
...
export interface SupportSkillUpsertEffectPayload
  extends PreToolEffectPayloadBase {
  readonly directory: string;
  readonly sessionId: string;
  readonly skillName: string;
...
export type PreToolEffectPayload =
  | TraceSkillAttemptEffectPayload
  | ForceDelegationAttemptEffectPayload
  | SupportSkillUpsertEffectPayload
  | ModeConfirmEffectPayload
  | AdvisoryClaimEffectPayload
  | AskUserNotifyEffectPayload;

export type PreToolEffectType =
