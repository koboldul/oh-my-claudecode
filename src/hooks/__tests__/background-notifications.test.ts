...
describe("dispatchNotificationInBackground", () => {
  let root: string;
  let childEntrypointPath: string;
  let hookRuntimePath: string;
  let child: EventEmitter & {
    connected: boolean;
    disconnect: typeof disconnectMock;
    kill: typeof killMock;
    send: typeof sendMock;
    unref: typeof unrefMock;
  };
  ...
  it("spawns detached notification work with ignored stdio", async () => {
    ...
    const result = await dispatchNotificationInBackground(
      "session-start",
      {
        sessionId: "sess-1",
  ...
  it("returns failed for an asynchronous spawn error", async () => {
    ...
    await expect(dispatchNotificationInBackground(
      "ask-user-question",
      { sessionId: "sess-1" },
      { childEntrypointPath, hookRuntimePath },
  ...
  it("times out when the child acknowledges neither spawn nor error", async () => {
    ...
    const result = dispatchNotificationInBackground(
      "ask-user-question",
      { sessionId: "sess-1" },
      { childEntrypointPath, hookRuntimePath },
    );
  ...
  it("does not derive child modules from import.meta.url", () => {
    const source = readFileSync(
      new URL("../background-notifications.ts", import.meta.url),
      "utf8",
    );
