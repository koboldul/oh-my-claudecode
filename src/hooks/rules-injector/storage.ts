...
function emptyState(sessionId: string): RulesState {
  return {
    sessionId,
...
function normalizeReservations(value: unknown): InjectedRuleReservation[] {
  if (!Array.isArray(value)) return [];

  const reservations: InjectedRuleReservation[] = [];
...
function parseState(
  ...
  const data = JSON.parse(raw) as Partial<InjectedRulesData>;
  return {
    sessionId,
...
function readStateUnlocked(
  ...
  if (!existsSync(filePath)) return emptyState(sessionId);

  try {
    return parseState(sessionId, readFileSync(filePath, 'utf-8'));
  } catch (error) {
    if (!strict) return emptyState(sessionId);
...
function pruneExpiredReservations(
  state: RulesState,
  now: number,
  preserveReservationId?: string,
): boolean {
  const retained = state.reservations.filter(
    (reservation) =>
      reservation.id === preserveReservationId
...
function writeStateUnlocked(state: RulesState): void {
  const filePath = getStoragePath(state.sessionId);
  if (
    state.contentHashes.size === 0
...
export function reserveInjectedRules(
  sessionId: string,
  candidates: readonly PlannedRuleToInject[],
): RulesInjectionReservation {
  ...
  return withRulesStateLock(sessionId, () => {
    ...
    const reservedRules: PlannedRuleToInject[] = [];
    for (const candidate of candidates) {
      if (
        unavailableHashes.has(candidate.contentHash)
    ...
    if (reservedRules.length === 0) {
      if (pruned) writeStateUnlocked(state);
      return { rules: [] };
    }
...
export function commitInjectedRuleReservation(
  sessionId: string,
  reservationId: string,
  deliveredRules: readonly PlannedRuleToInject[],
): void {
  withRulesStateLock(sessionId, () => {
    ...
    if (reservationIndex === -1) {
      throw new Error(
        `Rules delivery reservation "${reservationId}" is unavailable.`,
      );
    }
    ...
    for (const rule of deliveredRules) {
      if (!reservedKeys.has(identityKey(rule))) {
        throw new Error(
          `Rule "${rule.realPath}" was not part of reservation "${reservationId}".`,
        );
      }
...
export function releaseInjectedRuleReservation(
  sessionId: string,
  reservationId: string,
): void {
  withRulesStateLock(sessionId, () => {
