 ...
 */
export function isCopilotStatuslinePayload(value: unknown): boolean {
  if (!value || typeof value !== 'object') return false;
  const payload = value as Record<string, unknown>;

  const workspace = payload.workspace;
  const hasWorkspaceCurrentDir =
    !!workspace &&
    typeof workspace === 'object' &&
    typeof (workspace as Record<string, unknown>).current_dir === 'string';
 ...
 */
export function adaptCopilotStatusline(payload: Record<string, unknown>): StatuslineStdin {
  const result: StatuslineStdin = {};
  ...
  if (model && typeof model === 'object') {
    ...
    const mappedModel: NonNullable<StatuslineStdin['model']> = {};
  ...
  if (contextWindow && typeof contextWindow === 'object') {
    ...
    const mappedContextWindow: NonNullable<StatuslineStdin['context_window']> = {};
    ...
    if (currentUsage && typeof currentUsage === 'object') {
      const currentUsageRecord = currentUsage as Record<string, unknown>;
      const mappedCurrentUsage: NonNullable<
        NonNullable<StatuslineStdin['context_window']>['current_usage']
      > = {};
 ...
 */
export function normalizeStatuslineStdin(payload: unknown): StatuslineStdin {
  if (isCopilotStatuslinePayload(payload)) {
    return adaptCopilotStatusline(payload as Record<string, unknown>);
  }
  return payload as StatuslineStdin;
}
