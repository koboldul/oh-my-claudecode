...
export type CopilotStatusLineOwnership =
  | "missing"
  | "omc"
  | "third-party"
  | "invalid";
...
interface CopilotHudSnapshot extends CopilotHudStatus {
...
function getDefaultPackageRoot(): string {
  ...
  while (true) {
    if (
      ...
      && existsSync(
        join(candidate, "scripts", "lib", "hud-wrapper-template.txt"),
      )
    ) {
      return candidate;
    }
...
export function buildCopilotStatusLineCommand(
  nodePath: string,
  wrapperPath: string,
): string {
...
function updateStatusLineJsonc(
  content: string,
  ...
  if (
    ...
    let updated = content;
    const existing = existingStatusLine as Record<string, unknown>;
...
function buildSnapshot(options: CopilotHudSetupOptions): CopilotHudSnapshot {
  ...
  const wrapperCurrent =
    wrapperContent !== null && installedWrapper === wrapperContent;
  const helperCurrent =
    helperContent !== null && installedHelper === helperContent;
...
function toPublicStatus(snapshot: CopilotHudSnapshot): CopilotHudStatus {
  const {
...
export function configureCopilotHud(
  options: CopilotHudSetupOptions = {},
): CopilotHudSetupResult {
  const before = buildSnapshot(options);
  const replaceExisting = options.replaceExisting === true;
  const replacedThirdParty =
    before.ownership === "third-party" && replaceExisting;

  if (
    !before.settingsValid
    || (before.ownership === "third-party" && !replaceExisting)
  ...
  if (
    before.ownership !== "omc"
    || !matchesExpectedStatusLine(
...
type CopilotHudAction = "setup" | "repair" | "status" | "doctor";

function printHumanStatus(
  action: CopilotHudAction,
  result: CopilotHudStatus | CopilotHudSetupResult,
): void {
...
function runCli(args: string[]): number {
  ...
  const action = (
    args.find((arg, index) =>
      !arg.startsWith("-")
      && (pluginDirIndex < 0 || index !== pluginDirIndex + 1)
    ) ?? "status"
  ) as CopilotHudAction;
  ...
  const options: CopilotHudSetupOptions = {
