...
describe('cli-detection', () => {
  ...
  it('preserves the legacy detectAllClis keys', () => {
    ...
    expect(Object.keys(detectAllClis())).toEqual(['claude', 'codex', 'gemini', 'cursor', 'grok', 'antigravity', 'copilot']);
    expect(mockSpawnSync).toHaveBeenNthCalledWith(1, expectedFinder, ['claude'], expect.objectContaining({ shell: false }));
    expect(mockSpawnSync).toHaveBeenNthCalledWith(6, expectedFinder, ['agy'], expect.objectContaining({ shell: false }));
    expect(mockSpawnSync).toHaveBeenNthCalledWith(7, expectedFinder, ['copilot'], expect.objectContaining({ shell: false }));
  });
});
