...
let currentProcessStartIdentitySync: string | undefined;

interface WindowsDmtfTimestamp {
  epochMilliseconds: number;
  epochMicroseconds: bigint;
}

function parseWindowsDmtfTimestamp(
  value: string,
): WindowsDmtfTimestamp | undefined {
  ...
  return {
    epochMilliseconds,
    epochMicroseconds:
      BigInt(epochMilliseconds) * 1_000n + BigInt(microseconds),
  };
}

export function parseWindowsProcessStartIdentity(
  value: string,
): string | undefined {
 ...
 */
export function getProcessStartIdentitySync(
  pid: number,
): ProcessStartIdentitySync {
  ...
  let identity: ProcessStartIdentitySync = null;
