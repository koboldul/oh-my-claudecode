...
function identityForFd(fd) {
  const stat = (0, import_fs5.fstatSync)(fd);
  return { dev: stat.dev, ino: stat.ino };
}
function identityForPath(lockPath) {
  try {
    const stat = (0, import_fs5.lstatSync)(lockPath);
    return { dev: stat.dev, ino: stat.ino };
  } catch {
    return null;
  }
...
function observedLockStillMatches(lockPath, observation) {
  ...
  try {
    return (0, import_fs5.readFileSync)(lockPath, "utf-8") === observation.raw;
  } catch {
    return false;
  }
}
...
function recoveryOperationHasContender(recoveryPath, ownClaimPath) {
  ...
  let names;
  try {
    names = (0, import_fs5.readdirSync)(directory);
  } catch {
    return true;
  }
...
function createOwnedLock(lockPath, suppliedOwner) {
  ...
  const fd = (0, import_fs5.openSync)(
    lockPath,
    import_fs5.constants.O_CREAT | import_fs5.constants.O_EXCL | import_fs5.constants.O_RDWR,
    384
  );
  ...
  try {
    ...
    while (written < ownerBytes.length) {
      const count = (0, import_fs5.writeSync)(
        fd,
        ownerBytes,
        written,
    ...
    while (read < verifiedBytes.length) {
      const count = (0, import_fs5.readSync)(
        fd,
        verifiedBytes,
        read,
...
function createOwnedLockAtomically(lockPath, suppliedOwner) {
  ...
  let linked = false;
  try {
    (0, import_fs5.linkSync)(publicationPath, lockPath);
    linked = true;
    const published = {
      ...publication,
      path: lockPath
    ...
    try {
      if (handleStillOwnsPath(publication)) {
        (0, import_fs5.unlinkSync)(publicationPath);
      }
    } catch {
    }
    return published;
    ...
    if (linked) {
      ...
      try {
        if (handleStillOwnsPath(published)) (0, import_fs5.unlinkSync)(lockPath);
      } catch {
      }
    }
...
function handleStillOwnsPath(handle) {
  ...
  try {
    const raw = (0, import_fs5.readFileSync)(handle.path, "utf-8");
