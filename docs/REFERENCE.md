...
Manifest exposure and behavioral parity are separate claims. Copilot agent profiles declare **`claude-opus-5` with `max` reasoning** (set by `scripts/generate-copilot-agents.mjs`), and direct custom-agent selection uses those profiles. This is distinct from the Copilot *provider* default `gpt-5.6-sol` (`externalModels.defaults.copilotModel`), which drives `omc ask copilot`, Copilot team workers, and delegated Task/Agent defaulting in the pre-tool enforcer; both surfaces share `max` reasoning effort. Copilot CLI 1.0.72-1 delivers `PreToolUse` as a batched native `preToolUse` payload with `toolCalls[]`, so OMC must adapt per-call inputs and outputs before delegated Task/Agent defaulting through `updatedInput` can be claimed as equivalent to Claude Code.
...
| `OMC_COPILOT_DEFAULT_MODEL`      | `gpt-5.6-sol`        | Legacy/short override for Copilot advisor, Team workers, and Copilot-hosted OMC subagents                                                                                                                                                                                |
| `OMC_EXTERNAL_MODELS_DEFAULT_COPILOT_MODEL` | `gpt-5.6-sol` | Canonical Copilot model override; takes precedence over `OMC_COPILOT_DEFAULT_MODEL`                                                                                                                                                                                       |
| `OMC_COPILOT_REASONING_EFFORT`   | `max`                | Copilot reasoning effort: `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, or `max`                                                                                                                                                                                  |
...
omc ask copilot "review this patch with GPT-5.6 Sol"
...
- Provider matrix: `claude | copilot | codex | gemini | antigravity | grok | cursor`
...
- Skill entrypoint: `/oh-my-claudecode:ask <claude|copilot|codex|gemini|antigravity|grok|cursor> <prompt>` routes to this command

### `omc team` (CLI runtime surface)

```bash
omc team 2:codex "review auth flow"
omc team 2:copilot "implement and verify the auth flow"
...
- Provide authentication through runner environment variables or pre-authenticated provider CLIs for `copilot`, `codex`, `gemini`, `antigravity`, `grok`, or `cursor` when using `omc ask` / `omc team`.
...
| `ask`                     | Ask Claude, Copilot, Codex, Gemini, Antigravity, Grok, or Cursor via local CLI | `/oh-my-claudecode:ask`                     |
...
| **PostToolUse**        | `post-tool-verifier.mjs`, `project-memory-posttool.mjs`, `post-tool-rules-injector.mjs`                          | 5s, 3s, 3s       |
